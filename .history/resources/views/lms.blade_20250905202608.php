    ...
    <div class="container mx-auto px-4 relative z-10">
        ...
        <div class="max-w-7xl mx-auto">
            <div class="relative roadmap-timeline">
                ...
                <div class="relative flex items-center mb-20 last:mb-0" data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
                    <!-- Enhanced Timeline dot with number and glow -->
                    <div class="absolute left-1/2 transform -translate-x-1/2 z-20 timeline-dot">
                        <div class="relative">
                            <!-- Outer glow ring -->
                            <div class="w-20 h-20 bg-gradient-to-br {{ $path['color'] }} rounded-full absolute inset-0 blur-lg opacity-30 animate-pulse float-animation"></div>
                            <!-- Main circle -->
                            <div class="w-16 h-16 bg-gradient-to-br {{ $path['color'] }} rounded-full border-4 border-white shadow-2xl flex items-center justify-center relative transform hover:scale-110 transition-all duration-500 glow-blue">
                    ...
                    <div class="w-full {{ $path['position'] === 'left' ? 'pr-8 md:pr-16' : 'pl-8 md:pl-16' }} {{ $path['position'] === 'right' ? 'md:ml-auto' : '' }}">
                        <div class="roadmap-card bg-gradient-to-br {{ $path['bgColor'] }} backdrop-blur-sm border border-white/50 rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-500 {{ $path['position'] === 'left' ? 'md:text-right' : '' }} max-w-xl {{ $path['position'] === 'right' ? 'md:ml-auto' : '' }} group hover:border-white/80 relative overflow-hidden">
                            
                            <!-- Background decoration -->
                            <div class="absolute top-0 {{ $path['position'] === 'left' ? 'left-0' : 'right-0' }} w-32 h-32 bg-gradient-to-br {{ $path['color'] }} opacity-10 rounded-full blur-3xl transform {{ $path['position'] === 'left' ? '-translate-x-8' : 'translate-x-8' }} -translate-y-8 float-animation-reverse"></div>
                            
                            <div class="relative z-10">
                                <!-- Enhanced Phase badge with icon -->
                                <div class="inline-flex items-center gap-3 bg-white/90 backdrop-blur-sm text-slate-700 px-6 py-3 rounded-full text-sm font-bold mb-6 shadow-lg border border-white/50 group-hover:scale-105 transition-all duration-300">
                                ...
                                <h3 class="text-3xl font-black text-slate-900 mb-4 group-hover:bg-gradient-to-r group-hover:{{ $path['color'] }} group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300 leading-tight">
                                    {{ $path['title'] }}
                                </h3>
                                <p class="text-lg text-slate-600 mb-6 leading-relaxed">{{ $path['description'] }}</p>

                                <!-- Progress bar -->
                                <div class="mb-6">
                                    <div class="flex justify-between items-center mb-2">
                                        <span class="text-sm font-semibold text-slate-700">Progress Pembelajaran</span>
                                        <span class="text-sm font-bold bg-gradient-to-r {{ $path['color'] }} bg-clip-text text-transparent">{{ $path['progress'] }}%</span>
                                    </div>
                                    <div class="w-full bg-white/60 rounded-full h-3 overflow-hidden shadow-inner progress-bar">
                                        <div class="bg-gradient-to-r {{ $path['color'] }} h-3 rounded-full transition-all duration-1000 shadow-lg" style="width: {{ $path['progress'] }}%"></div>
                                    </div>
                                </div>

                                <!-- Enhanced Modules list -->
                                <div class="space-y-3">
                                    <h4 class="text-sm font-bold text-slate-700 mb-3 uppercase tracking-wider">Modul Pembelajaran</h4>
                                    @foreach($path['modules'] as $moduleIndex => $module)
                                    <div class="flex items-center gap-3 text-sm {{ $path['position'] === 'left' ? 'md:justify-end' : '' }} module-item transition-transform duration-300" style="animation-delay: {{ $moduleIndex * 100 }}ms;">
                                        <div class="w-3 h-3 bg-gradient-to-r {{ $path['color'] }} rounded-full shadow-lg"></div>
                                        ...
                                        <div class="w-1 h-1 bg-slate-300 rounded-full opacity-60"></div>
                                    </div>
                                    @endforeach
                                </div>

                                <!-- CTA Button -->
                                <div class="mt-8 {{ $path['position'] === 'left' ? 'md:text-right' : '' }}">
                                    <button class="btn-roadmap inline-flex items-center gap-2 bg-gradient-to-r {{ $path['color'] }} text-white px-6 py-3 rounded-full font-bold text-sm shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300">
                                        @if($path['progress'] > 0)
