@extends('layouts_old.app')@extends('layouts_old.app')
        ...
        materials: [
            {
                ...
                pdfUrl: '/pdfs/pengenalan-ekonomi-maritim.pdf', // Contoh PDF
                completed: false
            },
            {
                id: 2,
                ...
                `,
                videoUrl: 'https://www.youtube.com/embed/qz0aGYrrlhU',
                pdfUrl: '/pdfs/potensi-hasil-laut-indonesia.pdf', // Contoh PDF
                completed: false
            },
            {
                id: 3,
                title: 'Ekosistem Bisnis Perikanan',
                type: 'pdf', // Material yang fokus ke PDF
                duration: '50 menit',
                content: `
                    ...
                    <div class="prose max-w-none">
                        ...
                        <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4">
                            <h4 class="font-semibold text-blue-800 mb-2">📖 Panduan Pembelajaran</h4>
                            <p class="text-blue-700 text-sm mb-2">
                                Silakan pelajari dokumen PDF di bawah ini yang berisi panduan lengkap tentang ekosistem bisnis perikanan.
                            </p>
                            <ul class="text-blue-700 text-sm space-y-1">
                                <li>• Rantai nilai perikanan dari hulu ke hilir</li>
                                <li>• Model bisnis yang berkelanjutan</li>
                                <li>• Analisis stakeholder dan partnership</li>
                                <li>• Strategi integrasi vertikal dan horizontal</li>
                            </ul>
                        </div>
                    </div>
                `,
                videoUrl: 'https://www.youtube.com/embed/qz0aGYrrlhU',
                pdfUrl: '/pdfs/ekosistem-bisnis-perikanan.pdf', // PDF utama untuk material ini
                completed: false
            }
        ]
    },
...
function loadMaterial(index) {
    ...
    // Create material type indicator
    let typeIcon = '';
    let typeName = '';
    if (material.type === 'pdf') {
        typeIcon = '📄';
        typeName = 'Dokumen PDF';
    } else if (material.videoUrl) {
        typeIcon = '🎥';
        typeName = 'Video + Materi';
    } else {
        typeIcon = '📖';
        typeName = 'Materi Teks';
    }
    ...
    content.innerHTML = `
        <div class="fade-in">
                ...
                ${material.pdfUrl ? `
                    <div class="mb-8">
                        <div class="pdf-controls">
                            <div class="flex items-center gap-3">
                                <span class="text-sm font-semibold text-gray-700">📄 Dokumen PDF:</span>
                                <span class="text-sm text-gray-600">${material.title}</span>
                            </div>
                            <div class="flex gap-2">
                                <button onclick="togglePdfFullscreen('pdf-${index}')" class="px-3 py-1 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded text-sm font-medium transition-colors">
                                    🔍 Fullscreen
                                </button>
                                <a href="${material.pdfUrl}" download class="px-3 py-1 bg-green-100 hover:bg-green-200 text-green-700 rounded text-sm font-medium transition-colors">
                                    💾 Download
                                </a>
                                <a href="${material.pdfUrl}" target="_blank" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded text-sm font-medium transition-colors">
                                    🔗 Buka di Tab Baru
                                </a>
                            </div>
                        </div>
                        <div class="pdf-container" id="pdf-container-${index}">
                            <iframe id="pdf-${index}" src="${material.pdfUrl}#toolbar=1&navpanes=1&scrollbar=1" title="PDF: ${material.title}">
                                <div class="pdf-fallback">
                                    <div>
                                        <div class="text-6xl mb-4">📄</div>
                                        <h3 class="text-lg font-semibold text-gray-800 mb-2">PDF tidak dapat ditampilkan</h3>
                                        <p class="text-gray-600 mb-4">Browser Anda mungkin tidak mendukung embed PDF.</p>
                                        <a href="${material.pdfUrl}" target="_blank" class="btn-primary inline-block">
            ...
            ${material.pdfUrl && material.type === 'pdf' ? `
                <div class="bg-amber-50 border-l-4 border-amber-500 p-4 mt-6">
                    <h4 class="font-semibold text-amber-800 mb-2">💡 Tips Pembelajaran</h4>
                    <ul class="text-amber-700 text-sm space-y-1">
                        <li>• Gunakan mode fullscreen untuk membaca yang lebih nyaman</li>
                        <li>• Download PDF untuk akses offline</li>
                        <li>• Buat catatan penting saat membaca dokumen</li>
                        <li>• Jika PDF tidak muncul, klik "Buka di Tab Baru"</li>
                    </ul>
                </div>
            ` : ''}
...
function togglePdfFullscreen(pdfId) {
    const pdfContainer = document.getElementById('pdf-container-' + pdfId.split('-')[1]);
    const pdfIframe = document.getElementById(pdfId);
...
document.addEventListener('fullscreenchange', function() {
    if (!document.fullscreenElement) {
        // Reset PDF container height when exiting fullscreen
...
function handlePdfError(pdfId) {
    const pdfIframe = document.getElementById(pdfId);
    const container = pdfIframe.parentElement;
    ...
    container.innerHTML = `
        <div class="pdf-fallback">
            <div>
                ...
                <p class="text-gray-600 mb-4">File PDF mungkin tidak tersedia atau terjadi kesalahan jaringan.</p>
