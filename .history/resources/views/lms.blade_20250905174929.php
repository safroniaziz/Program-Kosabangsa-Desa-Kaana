...
<style>
    ...
    * {
    ...
    /* AOS fallback - ensure elements are visible if AOS doesn't load */
    [data-aos] {
        opacity: 1;
        transform: none;
    }
...
<!-- AOS Initialization -->
<script>
// Initialize AOS immediately when DOM is ready
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof AOS !== 'undefined') {
        ...
        console.log('❌ AOS not available, trying to load...');
        // Fallback: try to load AOS if not available
        ...
        script.onerror = function() {
            console.log('❌ Failed to load AOS');
        };
...
// Enhanced AOS initialization with better error handling
document.addEventListener('DOMContentLoaded', function() {
    // Wait a bit for all scripts to load
    setTimeout(function() {
        if (typeof AOS !== 'undefined') {
            console.log('🎯 Re-initializing AOS for better compatibility');
            AOS.init({
                duration: 1000,
                once: true,
                offset: 100,
            ...
            setTimeout(function() {
                AOS.refresh();
                console.log('🔄 AOS force refreshed');
                
                // Log all AOS elements for debugging
                const aosElements = document.querySelectorAll('[data-aos]');
                console.log(`📊 Found ${aosElements.length} AOS elements`);
                aosElements.forEach((el, i) => {
                    console.log(`${i+1}. ${el.getAttribute('data-aos')} - delay: ${el.getAttribute('data-aos-delay') || 'none'}`);
                });
            }, 200);
        } else {
            console.log('❌ AOS still not available after timeout');
        }
    }, 500);
});
