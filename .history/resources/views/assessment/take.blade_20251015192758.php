            ...
            <div class="bg-white rounded-xl shadow-lg p-6 md:p-8">
                ...
                <div class="mb-8">
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-3">
                        <div id="progressBar" class="bg-gradient-to-r {{ $assessment->type === 'pcl5' ? 'from-purple-500 to-purple-600' : 'from-green-500 to-green-600' }} h-3 rounded-full transition-all duration-500 ease-out" style="width: 0%"></div>
                    </div>
                ...
                <div class="questions-container space-y-8">
                    @foreach($questions['questions'] as $number => $question)
                        <div class="question-block p-6 bg-gray-50 rounded-xl border-2 border-transparent transition-all duration-300 hover:border-{{ $assessment->type === 'pcl5' ? 'purple' : 'green' }}-200" data-question="{{ $number }}">
                            <!-- Question Header -->
                            <div class="mb-6">
                                <div class="flex items-start space-x-4">
                                    <div class="flex-shrink-0 w-10 h-10 bg-{{ $assessment->type === 'pcl5' ? 'purple' : 'green' }}-100 text-{{ $assessment->type === 'pcl5' ? 'purple' : 'green' }}-700 rounded-full flex items-center justify-center text-sm font-bold shadow-sm">
                                        {{ $number }}
                                    </div>
                            ...
                            <div class="ml-14 space-y-3">
                                @foreach($questions['scale'] as $value => $label)
                                    <label class="answer-option group flex items-center p-4 bg-white border-2 border-gray-200 rounded-xl hover:border-{{ $assessment->type === 'pcl5' ? 'purple' : 'green' }}-300 hover:shadow-md cursor-pointer transition-all duration-200 relative">
                                        <input type="radio"
                                               name="answers[{{ $number }}]"
                                               value="{{ $value }}"
                                               class="w-5 h-5 text-{{ $assessment->type === 'pcl5' ? 'purple' : 'green' }}-600 bg-white border-2 border-gray-300 focus:ring-{{ $assessment->type === 'pcl5' ? 'purple' : 'green' }}-500 focus:ring-2 focus:ring-offset-2"
