        ...
        <form action="{{ route('assessment.submit', $assessment) }}" method="POST" id="assessmentForm" data-aos="fade-up" data-aos-delay="200">
            ...
            <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 md:p-10">
                                    ...
                                    <div class="px-8 py-6">
                                        @if($assessment->type === 'ptsd' || $assessment->type === 'ptsd_diagnostic')
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <input type="hidden" name="answers[{{ $number }}]" value="0" id="hidden_{{ $number }}">

                                                <label class="answer-option group block relative">
                                                    <input type="checkbox"
                                                           id="checkbox_{{ $number }}"
                                                           class="sr-only"
                                                           onchange="updateProgress(); animateSelection(this); toggleCheckboxVisual(this); updateCheckboxValue({{ $number }});">
                ...
                <div class="text-center mt-12 pt-8 border-t border-gray-200">
                    <button type="submit"
                            id="submitButton"
                            class="inline-flex items-center justify-center px-8 py-3 @if($assessment->type === 'checklist_masalah') bg-emerald-500 hover:bg-emerald-600 border-emerald-500 hover:border-emerald-600 @else bg-gray-400 hover:bg-gray-500 border-gray-400 hover:border-gray-500 @endif
...
<script>
    ...
    function updateCircularProgress(percent, answeredQuestions = 0, totalQuestions = 0) {
        ...
        @if($assessment->type === 'checklist_masalah' || $assessment->type === 'ptsd' || $assessment->type === 'ptsd_diagnostic')
            // For DCM and PTSD: enable submit only after user starts answering (at least 1 answer)
            if (answeredQuestions > 0) {
                submitButton.disabled = false;
                submitButton.classList.remove('opacity-50', 'cursor-not-allowed', 'bg-gray-400', 'hover:bg-gray-500', 'border-gray-400', 'hover:border-gray-500');
    ...
    function updateCheckboxValue(questionNumber) {
        const checkbox = document.getElementById('checkbox_' + questionNumber);
        const hiddenInput = document.getElementById('hidden_' + questionNumber);

        if (checkbox && hiddenInput) {
            // Set hidden input value based on checkbox state
            hiddenInput.value = checkbox.checked ? '1' : '0';
            console.log(`Question ${questionNumber}: checkbox=${checkbox.checked}, hidden value=${hiddenInput.value}`);
        }
    }
    ...
    document.getElementById('assessmentForm').addEventListener('submit', function(e) {
        ...
        @if($assessment->type === 'checklist_masalah')
            // For DCM: Check if at least 1 checkbox is checked
            answeredQuestions = document.querySelectorAll('input[type="checkbox"]:checked').length;
            ...
            if (answeredQuestions < 1) {
                Swal.fire({
                    ...
                    text: 'Mohon centang minimal 1 pernyataan sebelum menyelesaikan assessment.',
                    confirmButtonColor: '#3b82f6',
                    confirmButtonText: 'OK'
                });
                return false;
            }
        @elseif($assessment->type === 'ptsd' || $assessment->type === 'ptsd_diagnostic')
            // For PTSD: Check if at least 1 checkbox is checked
            answeredQuestions = document.querySelectorAll('input[type="checkbox"]:checked').length;
        ...
        @else
            // For other assessments (radio): require all questions to be answered
            answeredQuestions = document.querySelectorAll('input[type="radio"]:checked').length;
            ...
            if (answeredQuestions < totalQuestions) {
                Swal.fire({
                    ...
                    text: 'Mohon jawab semua pertanyaan sebelum menyelesaikan assessment.',
                    confirmButtonColor: '#3b82f6',
                    confirmButtonText: 'OK'
                });
                return false;
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-left">
                    ...
                    <div class="bg-blue-50 rounded-lg p-3 mb-3">
                        <p class="text-sm text-blue-800"><strong>Pertanyaan dijawab:</strong> ${answeredQuestions}/${totalQuestions}</p>
                        <p class="text-sm text-blue-800"><strong>Waktu estimasi:</strong> {{ $assessment->time_limit_minutes }} menit</p>
                    </div>
                    <p class="text-sm text-gray-600">Setelah submit, Anda tidak dapat mengubah jawaban lagi.</p>
                </div>
            `,
            icon: 'question',
            ...
            confirmButtonText: 'Ya, Selesaikan Assessment',
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                    ...
                    text: 'Hasil assessment Anda telah tersimpan. Anda akan diarahkan ke halaman riwayat.',
                    confirmButtonColor: '#10b981',
                    confirmButtonText: 'OK'
                }).then(() => {
                    // Show loading state
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if(session('success'))
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Redirect to results page or start-new page
                window.location.href = '{{ route('assessment.start-new', $assessment->id) }}';
            }
        });
    @endif
    ...
    @if($errors->any())
        Swal.fire({
            ...
            html: `
                <div class="text-left">
                    <p class="mb-3">Terdapat kesalahan dalam data yang dikirim:</p>
...
$(document).ready(function() {
    ...
    function addCounselorResponse(userMessage) {
        ...
        // Respons spesifik berdasarkan pesan
        if (userMessage.toLowerCase().includes('cemas')) {
            response = "Saya memahami rasa cemas yang Anda rasakan. Kecemasan adalah respons normal terhadap stres. Cobalah teknik pernapasan dalam: tarik napas 4 hitungan, tahan 4 hitungan, hembuskan 4 hitungan. Apakah Anda ingin berbicara lebih lanjut tentang apa yang membuat Anda cemas?";
        } else if (userMessage.toLowerCase().includes('tidur')) {
            response = "Kesulitan tidur dapat mempengaruhi kesehatan mental. Beberapa tips: hindari kafein 6 jam sebelum tidur, buat rutinitas tidur yang konsisten, dan hindari layar elektronik 1 jam sebelum tidur. Sudah berapa lama Anda mengalami kesulitan tidur?";
        } else if (userMessage.toLowerCase().includes('motivasi')) {
            response = "Saya bangga Anda mencari motivasi! Ingat, perjalanan seribu mil dimulai dari satu langkah. Fokus pada progres kecil setiap hari. Apa target kecil yang ingin Anda capai hari ini?";
        } else if (userMessage.toLowerCase().includes('stres')) {
            response = "Mengatasi stres sangat penting. Beberapa cara efektif: olahraga ringan 20-30 menit/hari, praktik mindfulness, berbicara dengan orang terdekat, dan istirahat yang cukup. Apa sumber stres utama Anda saat ini?";
        } else if (userMessage.toLowerCase().includes('cerita')) {
            response = "Saya mendengarkan dengan saksama. Ruang ini aman untuk Anda berbagi. Ceritakan apa yang ada di pikiran Anda, saya di sini untuk mendengarkan tanpa menghakimi.";
        } else if (userMessage.toLowerCase().includes('selanjutnya')) {
            response = "Berdasarkan hasil assessment Anda, saya sarankan: 1) Lakukan self-care rutin, 2) Praktikkan teknik relaksasi, 3) Jaga pola tidur yang baik, 4) Pertimbangkan konsultasi profesional jika gejala berlanjut. Apakah ada yang ingin Anda diskusikan lebih detail?";
        } else {
            // Respons umum
