...
<div id="chatConseling" class="fixed top-0 right-0 h-full w-96 bg-white shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out z-50 border-l border-gray-200">
    ...
    <!-- Chat Messages Area -->
    <div id="chatMessages" class="flex-1 overflow-y-auto p-3 bg-gray-50" style="height: calc(100vh - 300px);">
        <!-- Welcome Message -->
        <div class="mb-3">
    ...
    <div class="p-2 bg-gray-100 border-t">
        <p class="text-xs text-gray-600 mb-1">Respon Cepat:</p>
        <div class="space-y-1" id="quickResponseContainer">
...
$(document).ready(function() {
    ...
    function updateQuickResponses() {
        ...
        if (assessmentContext) {
            ...
            if (assessmentContext.averageScore < 2.5) {
                // Untuk tingkat kesiapan rendah - fokus pada dasar-dasar
                quickResponses = [
                    { text: '😰 Saya sering merasa cemas', message: 'Saya sering merasa cemas dan takut menghadapi situasi sulit. Bagaimana cara mengatasinya?' },
                    { text: '🆘 Butuh bantuan dasar', message: 'Saya butuh panduan dasar untuk mempersiapkan diri menghadapi situasi darurat' },
                    { text: '🧘 Teknik menenangkan diri', message: 'Ajari saya teknik sederhana untuk menenangkan diri saat panik' },
                    { text: '👥 Mencari dukungan', message: 'Bagaimana cara mencari dan meminta dukungan dari orang lain?' }
                ];
            } else if (assessmentContext.averageScore < 3.5) {
                // Untuk tingkat kesiapan menengah - fokus pada peningkatan
                quickResponses = [
                    { text: '💪 Meningkatkan resiliensi', message: 'Bagaimana cara meningkatkan resiliensi mental saya?' },
                    { text: '📋 Rencana persiapan', message: 'Bantuan saya membuat rencana persiapan yang lebih baik' },
                    { text: '🌟 Strategi coping', message: 'Saya ingin belajar strategi coping yang lebih efektif' },
                    { text: '🤝 Membantu orang lain', message: 'Bagaimana cara membantu orang lain yang sedang stress?' }
                ];
            } else {
                // Untuk tingkat kesiapan tinggi - fokus pada pemeliharaan dan kepemimpinan
                quickResponses = [
                    { text: '🎯 Mempertahankan kesiapan', message: 'Bagaimana cara mempertahankan tingkat kesiapan mental yang baik?' },
                    { text: '👨‍🏫 Menjadi mentor', message: 'Saya ingin membantu melatih orang lain untuk siap menghadapi bencana' },
                    { text: '🔄 Evaluasi berkala', message: 'Kapan sebaiknya saya mengevaluasi ulang kesiapan mental saya?' },
                    { text: '🌐 Membangun komunitas', message: 'Bagaimana membangun komunitas siaga bencana yang solid?' }
                ];
            }
            ...
            quickResponses.forEach(response => {
                buttonsHTML += `
                    <button onclick="sendQuickMessage('${response.message}')" class="w-full text-left p-1.5 bg-white rounded text-xs hover:bg-blue-50 transition-colors border border-gray-200">
                        ${response.text}
                    </button>
                `;
            });
