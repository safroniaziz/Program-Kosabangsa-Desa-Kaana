...
<section class="py-20 bg-white">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        <div class="text-center mb-16" data-aos="fade-up">
            <span class="text-sm font-bold text-blue-600 tracking-wider uppercase">FAQ</span>
            <h2 class="text-4xl lg:text-5xl font-black text-slate-900 mt-4 mb-6">
                Pertanyaan yang
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600">Sering Diajukan</span>
            </h2>
        </div>
        ...
        <div class="space-y-4" data-aos="fade-up" data-aos-delay="200">
            ...
            $faqs = [
                [
                    ...
                    'answer' => 'Ya, kursus ini dirancang untuk pemula hingga level advanced. Kami mulai dari dasar-dasar HTML/CSS hingga konsep advanced Full-Stack Development.'
                ],
                [
                    'question' => 'Berapa lama waktu yang dibutuhkan untuk menyelesaikan kursus?',
                    'answer' => 'Rata-rata siswa menyelesaikan kursus dalam 3-6 bulan dengan belajar 2-3 jam per hari. Namun, Anda bisa belajar sesuai kecepatan Anda sendiri.'
                ],
                [
                    'question' => 'Apakah ada sertifikat setelah menyelesaikan kursus?',
                    'answer' => 'Ya, Anda akan mendapatkan sertifikat digital yang bisa di-share di LinkedIn atau CV Anda setelah menyelesaikan semua modul dan project.'
                ],
                [
                    'question' => 'Apakah ada support atau mentoring?',
                    'answer' => 'Ya, kami menyediakan forum diskusi dan sesi mentoring mingguan untuk membantu Anda mengatasi kesulitan dalam belajar.'
                ]
            ];
            @endphp
            ...
            <div class="border border-slate-200 rounded-xl overflow-hidden">
                <button onclick="toggleFAQ({{ $index }})" class="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-slate-50 transition-colors">
                    <span class="font-semibold text-slate-900">{{ $faq['question'] }}</span>
                    <svg id="faq-icon-{{ $index }}" class="w-5 h-5 text-slate-500 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<section class="py-20 bg-gradient-to-br from-blue-600 to-purple-600 relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        <h2 class="text-4xl lg:text-5xl font-black text-white mb-6" data-aos="fade-up">
            Siap Memulai Karir sebagai
        ...
        <p class="text-xl text-white/90 mb-8 max-w-2xl mx-auto" data-aos="fade-up" data-aos-delay="100">
            Bergabunglah dengan ribuan developer sukses yang telah mengubah karir mereka melalui kursus ini
        </p>
        <div class="flex flex-col sm:flex-row gap-4 justify-center" data-aos="zoom-in" data-aos-delay="200">
            <button onclick="scrollToModules()" class="bg-white text-blue-600 px-8 py-4 rounded-xl font-bold text-lg hover:shadow-2xl transition-all hover:scale-105">
                Mulai Belajar Sekarang
            </button>
            <button class="bg-transparent border-2 border-white text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-white/10 transition-all">
                Konsultasi Gratis
            </button>
        </div>
            ...
            path.setAttribute('d', 'M8 5            svg.setAttribute('fill', 'currentColor');            icon.classList.add('video-icon');        } else {            path.setAttribute('d', 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z');            path.setAttribute('stroke-width', '2');            path.setAttribute('stroke-linejoin', 'round');            path.setAttribute('stroke-linecap', 'round');            svg.setAttribute('stroke', 'currentColor');            svg.setAttribute('fill', 'none');            icon.classList.add('text-icon');        if (material.type === 'text') {        const path = content.querySelector('.material-svg-path');        const svg = content.querySelector('.material-svg');        const icon = content.querySelector('.material-icon');        // Configure icon based on type        materialItem.onclick = () => loadMaterial(index);        // Set click handler        }            content.querySelector('.active-indicator').classList.remove('hidden');            materialItem.classList.add('active');        if (index === currentMaterialIndex) {        // Set active state        const materialItem = content.querySelector('.material-item');        const content = template.content.cloneNode(true);        const template = document.getElementById('materialItemTemplate');    currentMaterials.forEach((material, index) => {    materialsList.innerHTML = '';    const materialsList = document.getElementById('materialsList');function populateMaterialsList() {// Populate materials list - cleaned up, using template}    }            break;            }                loadMaterial(materialIndex);            if (materialIndex < currentMaterials.length) {            const materialIndex = parseInt(e.key) - 1;            e.preventDefault();        case '8':        case '7':        case '6':        case '5':        case '4':        case '3':        case '2':        case '1':            break;            closeModal();            e.preventDefault();        case 'Escape':            break;            nextMaterial();            e.preventDefault();        case 'ArrowRight':            break;            previousMaterial();            e.preventDefault();        case 'ArrowLeft':    switch(e.key) {    if (document.getElementById('learningModal').classList.contains('hidden')) return;function handleModalKeyboard(e) {// Enhanced keyboard navigation}    }, 300);        document.removeEventListener('keydown', handleModalKeyboard);        document.body.style.overflow = '';        modal.classList.add('hidden');    setTimeout(() => {    modalContent.classList.remove('show');    modalContent.classList.add('scale-95');    modal.classList.add('opacity-0');    // Animate modal disappearance    const modalContent = modal.querySelector('.modal-content');    const modal = document.getElementById('learningModal');function closeModal() {// Close modal function with smooth animation}    document.addEventListener('keydown', handleModalKeyboard);    // Add keyboard event listener    }        loadMaterial(0);    if (currentMaterials.length > 0) {    // Load first material if available    }, 10);        modalContent.classList.add('show');        modalContent.classList.remove('scale-95');        modal.classList.remove('opacity-0');    setTimeout(() => {    // Trigger animations    document.body.style.overflow = 'hidden';    modal.classList.remove('hidden');    const modalContent = modal.querySelector('.modal-content');    const modal = document.getElementById('learningModal');    // Show modal with stunning animation    populateMaterialsList();    // Populate materials list    document.getElementById('modalTitle').textContent = currentModule.title;    // Update modal title    currentMaterialIndex = 0;    currentMaterials = currentModule.materials_list || [];    if (!currentModule) return;    currentModule = moduleData;function openModule(moduleNumber, moduleData) {// Open module function with enhanced animations - now receives module data from PHP}    }        arrow.style.transform = 'rotate(0deg)';        materialsDiv.classList.add('hidden');    } else {        arrow.style.transform = 'rotate(180deg)';        materialsDiv.classList.remove('hidden');    if (materialsDiv.classList.contains('hidden')) {    const arrow = document.getElementById(`arrow-${moduleNumber}`);    const materialsDiv = document.getElementById(`materials-${moduleNumber}`);function toggleMaterials(moduleNumber) {// Toggle materials visibility}    });        behavior: 'smooth'    document.getElementById('modules').scrollIntoView({function scrollToModules() {// Function to scroll to modules sectionlet currentMaterials = [];let currentMaterialIndex = 0;let currentModule = null;// Global variables for learning modal<script>@section('scripts')@endsection</div>    </div>        </div>            </div>                </div>                    </div>                        </div>                            </div>                                </button>                                    </svg>                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>                                    <svg class="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">                                    <span>Selanjutnya</span>                                <button id="nextBtn" onclick="nextMaterial()" class="flex items-center gap-2 btn-primary text-white px-6 py-3 rounded-xl font-medium transition-all shadow-lg group">                                </button>                                    <span>Sebelumnya</span>                                    </svg>                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>                                    <svg class="w-4 h-4 group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">                                <button id="prevBtn" onclick="previousMaterial()" class="flex items-center gap-2 px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed group" disabled>                            <div class="flex items-center gap-3">                            </div>                                </div>                                    <span id="currentMaterialType" class="hidden px-3 py-1 bg-gradient-to-r from-blue-100 to-purple-100 text-slate-700 rounded-full text-xs font-medium border border-blue-200"></span>                                    <span id="currentMaterialDuration" class="text-slate-600">Durasi akan ditampilkan di sini</span>                                <div class="flex items-center gap-4 text-sm">                                <h4 id="currentMaterialTitle" class="text-xl font-bold text-slate-900 mb-2">Pilih materi untuk memulai</h4>                            <div class="flex-1">                        <div class="flex items-center justify-between">                    <div class="bg-white/90 backdrop-blur-sm border-t border-slate-200/50 p-6">                    <!-- Material Info & Navigation dengan design yang lebih menarik -->                    </template>                        </div>                            <div class="active-indicator hidden"></div>                            </div>                                <div class="material-badges"></div>                                <div class="material-duration"></div>                                <div class="material-title"></div>                            <div class="material-info">                            </div>                                </svg>                                    <path class="material-svg-path"></path>                                <svg class="material-svg w-4 h-4" viewBox="0 0 24 24">                            <div class="material-icon">                        <div class="material-item">                    <template id="materialItemTemplate">                    <!-- Template untuk material item -->                    </template>                        </div>                            </div>                                <span class="toast-message"></span>                                </svg>                                    <path class="toast-icon-path" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></path>                                <svg class="toast-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">                            <div class="toast-content">                        <div class="toast success">                    <template id="toastTemplate">                    <!-- Template untuk toast notification -->                    </template>                        </div>                            </div>                                <button class="back-to-material-btn">← Kembali ke Materi</button>                                <p class="video-message">Tonton video dengan seksama dan jangan ragu untuk pause jika diperlukan</p>                            <div class="video-controls">                            </div>                                </div>                                    <iframe class="video-iframe" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>                                <div class="video-frame">                            <div class="video-wrapper">                        <div class="video-player-container">                    <template id="videoPlayerTemplate">                    <!-- Template untuk video player -->                    </template>                        </div>                            </button>                                📥 Download File                            <button class="download-btn w-full bg-gradient-to-r from-blue-500 to-cyan-600 text-white py-3 px-4 rounded-xl font-medium hover:shadow-lg transition-all group-hover:scale-105">                            </div>                                </div>                                    <p class="file-name text-sm text-slate-600"></p>                                    <h4 class="font-bold text-slate-900">File Pendukung</h4>                                <div>                                </div>                                    </svg>                                        <path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z"/>                                    <svg class="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">                                <div class="w-12 h-12 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">                            <div class="flex items-center gap-4 mb-4">                        <div class="glass border border-white/20 rounded-2xl p-6 hover:shadow-xl transition-all group">                    <template id="fileResourceTemplate">                    <!-- Template untuk file resource -->                    </template>                        </div>                            </button>                                ▶️ Tonton Video                            <button class="video-btn w-full bg-gradient-to-r from-red-500 to-pink-600 text-white py-3 px-4 rounded-xl font-medium hover:shadow-lg transition-all group-hover:scale-105">                            </div>                                </div>                                    <p class="text-sm text-slate-600">Tonton untuk pemahaman lebih dalam</p>                                    <h4 class="font-bold text-slate-900">Video Pembelajaran</h4>                                <div>                                </div>                                    </svg>                                        <path d="M8 5v14l11-7z"/>                                    <svg class="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">                                <div class="w-12 h-12 bg-gradient-to-br from-red-500 to-pink-600 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">                            <div class="flex items-center gap-4 mb-4">                        <div class="glass border border-white/20 rounded-2xl p-6 hover:shadow-xl transition-all group">                    <template id="videoResourceTemplate">                    <!-- Template untuk video resource -->                    </template>                        </div>                            </div>                                </div>                                    </div>                                        </div>                                            <p class="learning-tips text-green-800 leading-relaxed"></p>                                            </h5>                                                💡 Tips Belajar                                                </svg>                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/>                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">                                            <h5 class="font-bold text-green-900 mb-3 flex items-center gap-2">                                        <div class="mt-8 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-6">                                        <!-- Tips section -->                                        <div class="additional-resources grid grid-cols-1 md:grid-cols-2 gap-6"></div>                                        </h3>                                            Sumber Belajar Tambahan                                            </div>                                                </svg>                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>                                                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">                                            <div class="w-8 h-8 bg-gradient-to-br from-green-500 to-emerald-600 rounded-lg flex items-center justify-center">                                        <h3 class="text-2xl font-bold text-slate-900 mb-6 flex items-center gap-3">                                    <div class="border-t pt-8 mt-8">                                    <!-- Additional resources section -->                                    <div class="material-content text-slate-700 leading-relaxed text-lg mb-8 space-y-6"></div>                                    </div>                                        <p class="material-description text-blue-800 leading-relaxed"></p>                                        </h4>                                            Tentang Materi Ini                                            </svg>                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">                                        <h4 class="text-xl font-bold text-blue-900 mb-3 flex items-center gap-2">                                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 border-l-4 border-blue-500 p-6 rounded-r-xl mb-8">                                <div class="prose prose-lg max-w-none">                                <!-- Content Body -->                                </div>                                    </div>                                        </div>                                            <div class="material-progress-bar bg-gradient-to-r from-blue-500 to-purple-600 h-2 rounded-full transition-all duration-500"></div>                                        <div class="w-full bg-white/80 rounded-full h-2">                                        </div>                                            <span class="material-progress text-sm font-bold text-blue-600"></span>                                            <span class="text-sm font-medium text-slate-700">Progress Materi</span>                                        <div class="flex items-center justify-between mb-2">                                    <div class="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200/50 rounded-xl p-4 mb-8">                                    <!-- Progress indicator -->                                    </div>                                        </div>                                            </div>                                                </span>                                                    📖 Teks                                                <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-medium">                                                <span class="w-1 h-1 bg-slate-400 rounded-full"></span>                                                </span>                                                    <span class="material-duration"></span>                                                    </svg>                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">                                                <span class="flex items-center gap-2">                                            <div class="flex items-center gap-4 text-slate-600">                                            <h1 class="material-title text-3xl font-black text-slate-900 mb-2"></h1>                                        <div>                                        </div>                                            <span class="material-number text-white font-bold text-lg"></span>                                        <div class="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center">                                    <div class="flex items-center gap-4 mb-6">                                <div class="mb-8">                                <!-- Material Header -->                            <div class="max-w-7xl mx-auto p-8">                        <div class="w-full h-full bg-white/70 backdrop-blur-sm overflow-y-auto">                    <template id="textContentTemplate">                    <!-- Template untuk text content -->                    </div>                        </div>                            </div>                                </div>                                    </div>                                        </p>                                            💡 <strong>Tips:</strong> Selesaikan materi secara berurutan untuk hasil pembelajaran yang optimal                                        <p class="text-sm text-slate-700">                                    <div class="mt-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200/50">                                    </p>                                        Pilih salah satu materi di sidebar untuk memulai perjalanan belajar yang menakjubkan!                                    <p class="text-slate-600 leading-relaxed">                                    <h3 class="text-2xl font-bold text-slate-900 mb-4">Mulai Pembelajaran Anda</h3>                                    </div>                                        </svg>                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>                                        <svg class="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">                                    <div class="w-24 h-24 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-xl">                                <div class="text-center max-w-md mx-auto p-8">                            <div class="flex items-center justify-center h-full">                            <!-- Default state dengan design yang lebih menarik -->                        <div id="contentArea" class="w-full h-full">                    <div class="flex-1 bg-white/70 backdrop-blur-sm relative overflow-y-auto">                    <!-- Content Display Area -->                <div class="flex-1 flex flex-col">                <!-- Content area dengan design modern -->                </div>                    </div>                        </div>                            <!-- Materials akan diisi oleh JavaScript -->                        <div id="materialsList" class="space-y-3">                        </h3>                            <span>Daftar Materi</span>                            </div>                                </svg>                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"/>                                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">                            <div class="w-8 h-8 bg-gradient-to-br from-purple-500 to-blue-600 rounded-lg flex items-center justify-center">                        <h3 class="text-lg font-bold text-slate-900 mb-6 flex items-center gap-3">                    <div class="p-6">                <div class="w-80 bg-slate-50/80 backdrop-blur-sm border-r border-slate-200/50 overflow-y-auto">                <!-- Materials sidebar dengan glass effect -->            <div class="flex h-[calc(90vh-180px)]">            <!-- Modal content with improved design -->            </div>                </div>                    </div>                        <div id="modalProgressBar" class="bg-gradient-to-r from-white via-white/80 to-white/60 h-3 rounded-full transition-all duration-1000 shadow-lg" style="width: 0%"></div>                    <div class="w-full bg-white/20 rounded-full h-3 overflow-hidden">                    </div>                        <span id="currentProgress" class="font-bold">0 dari 8 materi</span>                        <span class="font-medium">Progress Modul</span>                    <div class="flex justify-between text-sm mb-2">                <div class="mt-6">                <!-- Progress bar dengan gradient -->                </div>                    </button>                        </svg>                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>                        <svg class="w-6 h-6 group-hover:rotate-90 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">                    <button onclick="closeModal()" class="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center border border-white/30 hover:bg-white/30 transition-all group">                    </div>                        </div>                            </div>                                <span class="font-medium">8 materi</span>                                </svg>                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">                            <div class="flex items-center gap-2">                            </div>                                <span class="font-medium">12 jam pembelajaran</span>                                </svg>                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">                            <div class="flex items-center gap-2">                        <div class="flex items-center gap-6 text-white/90">                        <h2 id="modalTitle" class="text-3xl font-black mb-2">HTML & CSS Fundamentals</h2>                    <div>                <div class="relative z-10 flex items-center justify-between">                </div>                    <div class="absolute inset-0" style="background-image: radial-gradient(circle at 20% 80%, white 1px, transparent 1px), radial-gradient(circle at 80% 20%, white 1px, transparent 1px); background-size: 30px 30px;"></div>                <div class="absolute inset-0 opacity-10">                <!-- Background pattern -->            <div class="relative gradient-bg-1 text-white p-8 overflow-hidden">            <!-- Modal header with glass effect -->        <div class="modal-content bg-white/95 backdrop-blur-sm rounded-3xl max-w-6xl w-full max-h-[90vh] overflow-hidden shadow-2xl border border-white/20 transform scale-95 transition-all duration-500">    <div class="flex items-center justify-center min-h-screen p-4"><div id="learningModal" class="fixed inset-0 bg-black/70 backdrop-blur-xl z-50 hidden opacity-0 transition-all duration-500"><!-- Learning Modal - Ultra Modern Design --></section>    </div>        </div>            </div>                <span>Sertifikat Resmi</span>                </svg>                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"/>                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">            <div class="flex items-center gap-2">            </div>                <span>Gratis 100%</span>                </svg>                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M12 10V3m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/>                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">            <div class="flex items-center gap-2">            </div>                <span>Akses Selamanya</span>                </svg>                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">            <div class="flex items-center gap-2">        <div class="mt-8 flex items-center justify-center gap-8 text-white" data-aos="fade-up" data-aos-delay="300">
