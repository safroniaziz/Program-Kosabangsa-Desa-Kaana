        ...
        <div class="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 text-white" data-aos="fade-up" data-aos-delay="600">
            <div class="text-center mb-8">
                <div class="flex items-center justify-center gap-3 mb-3">
                    <h3 class="text-2xl font-bold">Ringkasan Indeks Pembangunan Desa</h3>
                    <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $statusBgColors[$indices['status_color']] ?? 'bg-amber-50 border-amber-200' }} {{ $statusTextColors[$indices['status_color']] ?? 'text-amber-700' }} border">
                        Status: {{ $indices['status_label'] }}
                    </span>
                </div>
                <p class="text-blue-100 text-sm">Penilaian komprehensif berdasarkan {{ number_format($indices['total_population']) }} warga terdata</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                    <div class="flex items-center justify-between mb-3">
                        <div class="text-sm text-blue-200 font-medium">Indeks Pembangunan</div>
                        <svg class="w-5 h-5 text-blue-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                        </svg>
                    </div>
                    <div class="text-4xl font-bold mb-2" data-counter="{{ $indices['development_index'] }}">0</div>
                    <div class="text-xs text-blue-200 mb-3">Rata-rata: Kesejahteraan + Melek Huruf + Usia Produktif</div>
                    <div class="h-2 bg-white/20 rounded-full overflow-hidden">
                        <div class="h-full {{ $statusColors[$indices['status_color']] ?? 'bg-amber-500' }}" style="width: {{ $indices['development_index'] }}%"></div>
                    </div>
                    <p class="text-xs text-blue-300 mt-2">Dari data {{ number_format($indices['total_population']) }} warga terdaftar</p>
                </div>
                ...
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                    <div class="flex items-center justify-between mb-3">
                        <div class="text-sm text-blue-200 font-medium">Melek Huruf</div>
                        <svg class="w-5 h-5 text-blue-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                        </svg>
                    </div>
                    <div class="text-4xl font-bold mb-2" data-counter="{{ round($indices['literacy_percentage']) }}">0</div>
                    <div class="text-xs text-blue-200 mb-3">Warga dengan pendidikan (SD ke atas)</div>
                    <div class="h-2 bg-white/20 rounded-full overflow-hidden">
                        <div class="h-full bg-blue-400" style="width: {{ $indices['literacy_percentage'] }}%"></div>
                    </div>
                    <p class="text-xs text-blue-300 mt-2">Dari data tingkat pendidikan warga</p>
                </div>
