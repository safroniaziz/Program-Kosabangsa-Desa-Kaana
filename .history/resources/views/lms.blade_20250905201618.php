...
<section class="py-20 bg-white">
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto text-center mb-16" data-aos="fade-up" data-aos-duration="600">
            <div class="inline-block mb-4" data-aos="zoom-in" data-aos-delay="100">
                <span class="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 tracking-wider uppercase">Roadmap Pembelajaran</span>
            </div>
            <h2 class="text-4xl font-bold text-slate-900 mb-6" data-aos="fade-up" data-aos-delay="150">
                Path Pembelajaran Terstruktur
            </h2>
            <p class="text-xl text-slate-600 leading-relaxed" data-aos="fade-up" data-aos-delay="200">
        ...
        <div class="max-w-4xl mx-auto" data-aos="fade-up" data-aos-delay="400">
            <div class="relative">
                <!-- Vertical line -->
                <div class="absolute left-1/2 transform -translate-x-px h-full w-0.5 bg-gradient-to-b from-blue-300 via-blue-400 to-blue-500" data-aos="fade-down" data-aos-delay="500" data-aos-duration="1000"></div>
                ...
                <div class="relative flex items-center mb-16 last:mb-0"
                     data-aos="{{ $path['position'] === 'left' ? 'fade-right' : 'fade-left' }}"
                     data-aos-delay="{{ ($index * 200) + 600 }}"
                     data-aos-duration="800">
                    <!-- Timeline dot -->
                    <div class="absolute left-1/2 transform -translate-x-1/2 w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-700 rounded-full border-4 border-white shadow-xl flex items-center justify-center z-10 hover:scale-110 transition-transform duration-300"
                         data-aos="zoom-in"
                    ...
                    <div class="w-full {{ $path['position'] === 'left' ? 'pr-8 md:pr-12' : 'pl-8 md:pl-12' }} {{ $path['position'] === 'right' ? 'md:ml-auto' : '' }}">
                        <div class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 {{ $path['position'] === 'left' ? 'md:text-right' : '' }} max-w-md {{ $path['position'] === 'right' ? 'md:ml-auto' : '' }} group hover:border-blue-300"
                             data-aos="fade-up"
                             data-aos-delay="{{ ($index * 200) + 800 }}">
                            <!-- Phase badge -->
                            <div class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-50 to-blue-100 text-blue-700 px-4 py-2 rounded-full text-sm font-medium mb-4 group-hover:from-blue-100 group-hover:to-blue-200 transition-all duration-300"
                                 data-aos="fade-in"
                            ...
                            <h3 class="text-xl font-bold text-slate-900 mb-3 group-hover:text-blue-700 transition-colors duration-300"
                                data-aos="fade-in"
                                data-aos-delay="{{ ($index * 200) + 1000 }}">{{ $path['title'] }}</h3>
                            <p class="text-slate-600 mb-4 leading-relaxed"
                               data-aos="fade-in"
                               data-aos-delay="{{ ($index * 200) + 1100 }}">{{ $path['description'] }}</p>

                            <!-- Modules list -->
                            <div class="space-y-2" data-aos="fade-up" data-aos-delay="{{ ($index * 200) + 1200 }}">
                                @foreach($path['modules'] as $moduleIndex => $module)
                                <div class="flex items-center gap-2 text-sm {{ $path['position'] === 'left' ? 'md:justify-end' : '' }} group-hover:translate-x-1 transition-transform duration-300"
                                     data-aos="fade-{{ $path['position'] === 'left' ? 'left' : 'right' }}"
                                     data-aos-delay="{{ ($index * 200) + 1200 + ($moduleIndex * 100) }}">
                                    <div class="w-2 h-2 bg-gradient-to-r from-blue-400 to-blue-600 rounded-full animate-pulse"></div>
                                    <span class="text-slate-700 group-hover:text-blue-600 transition-colors duration-300">{{ $module }}</span>
                                </div>
                                @endforeach
                            </div>
                        </div>
