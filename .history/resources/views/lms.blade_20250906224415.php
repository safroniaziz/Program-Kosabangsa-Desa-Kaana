            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 group module-card"
                    ...
                    <div class="p-4 bg-white">
                        ...
                        <div class="space-y-3">
                            @if($module['status'] === 'available')
                            <button onclick="openLearningPage({{ $module['number'] }}, @json($module))" class="w-full group relative overflow-hidden bg-gradient-to-br from-emerald-500 via-cyan-500 to-blue-600 hover:from-emerald-600 hover:via-cyan-600 hover:to-blue-700 text-white px-6 py-4 rounded-2xl font-bold text-sm shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:scale-[1.02] hover:-translate-y-1">
                                <!-- Animated background gradient -->
                                <div class="absolute inset-0 bg-gradient-to-br from-emerald-400/80 via-cyan-400/80 to-blue-500/80 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                                
                                <!-- Shimmer wave effect -->
                                <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1200 ease-in-out"></div>
                                
                                <!-- Glow effect -->
                                <div class="absolute inset-0 bg-gradient-to-br from-emerald-400/50 via-cyan-400/50 to-blue-500/50 opacity-0 group-hover:opacity-100 blur-xl transition-opacity duration-500"></div>
                                
                                <!-- Pulse ring -->
                                <div class="absolute inset-0 border-2 border-white/20 rounded-2xl scale-100 group-hover:scale-105 opacity-0 group-hover:opacity-100 transition-all duration-500"></div>

                                <span class="relative z-10 flex items-center justify-center gap-3">
                                    <!-- Play icon with rotation effect -->
                                    <div class="relative">
                                        <svg class="w-6 h-6 group-hover:rotate-90 transition-all duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <!-- Icon glow -->
                                        <div class="absolute inset-0 bg-white/30 rounded-full scale-0 group-hover:scale-150 opacity-0 group-hover:opacity-100 transition-all duration-500"></div>
                                    </div>
                                    
                                    <span class="tracking-wider font-extrabold text-base">
                                        {{ $module['progress'] > 0 ? 'Lanjutkan Belajar' : 'Mulai Belajar' }}
                                    </span>
                                    
                                    <!-- Arrow with enhanced animation -->
                                    <svg class="w-5 h-5 group-hover:translate-x-2 group-hover:scale-110 transition-all duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 7l5 5m0 0l-5 5m5-5H6"/>
                                    </svg>
                                </span>

                                <!-- Decorative floating particles -->
                                <div class="absolute top-2 right-3 w-2 h-2 bg-white/40 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100"></div>
                                <div class="absolute bottom-3 left-3 w-1.5 h-1.5 bg-white/50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-200"></div>
                                <div class="absolute top-1/2 left-2 w-1 h-1 bg-white/60 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-300"></div>
                                <div class="absolute top-4 left-1/2 w-1 h-1 bg-white/40 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-400"></div>
                            </button>
                            @else
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.onclick || e.target.closest('[onclick]')) {
    ...
    // Fix for buttons with onclick attributes - ensure they work
    document.querySelectorAll('button[onclick]').forEach(button => {
        console.log('Found button with onclick:', button);
        button.addEventListener('click', function(e) {
            console.log('Button clicked:', this);
            console.log('Onclick attribute:', this.getAttribute('onclick'));
            // Prevent any potential interference
            e.stopPropagation();
            // Make sure the onclick attribute is executed
            if (this.onclick && typeof this.onclick === 'function') {
                this.onclick(e);
            } else if (this.getAttribute('onclick')) {
                // Execute the onclick attribute manually if needed
                try {
    ...
    // Also ensure direct function calls work
    window.scrollToModules = function() {
        console.log('Global scrollToModules called');
        const modulesSection = document.getElementById('modules');
        if (modulesSection) {
            modulesSection.scrollIntoView({
                behavior: 'smooth'
            });
            console.log('Scrolling to modules section');
        } else {
            console.error('Modules section not found');
        }
    };

    window.openLearningPage = function(moduleNumber, moduleData) {
        console.log('Opening learning page for module:', moduleNumber);
        // Redirect to learning page with module number
        window.location.href = `/learning/${moduleNumber}`;
    };

    window.openModule = function(moduleNumber, moduleData) {
        console.log('Global openModule called with:', moduleNumber, moduleData);
        currentModule = moduleData;
        if (!currentModule) {
            console.error('No module data provided');
            return;
        }

        const modal = document.getElementById('learningModal');
        if (!modal) {
            console.error('Learning modal not found');
            return;
        }
