    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <section class="py-20 bg-slate-50">
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
                <div class="text-center mb-16" data-aos="fade-up">
                    <span class="data-chip">Profil Demografi</span>
                    <h2 class="text-4xl md:text-5xl font-bold text-slate-900 mt-4">Analisis Komprehensif Demografi Desa</h2>
                    <p class="section-subtitle mt-4">Pemahaman mendalam tentang komposisi penduduk berdasarkan berbagai indikator untuk mendukung perencanaan pembangunan yang tepat sasaran.</p>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
                    <div class="chart-container" data-aos="fade-right" data-aos-delay="120">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-2xl font-semibold text-slate-900">Distribusi Agama</h3>
                            <span class="data-chip">Religiusitas</span>
                        </div>
                        <div class="flex items-center justify-center mb-8">
                            @php
                                $religionData = $chartData['religion_distribution'] ?? collect();
                                $totalPop = $chartData['total_population'] ?? 0;
                                $religionPercentages = [];
                                $religionColors = [];
                                $cumulative = 0;
                                foreach ($religionData as $rel) {
                                    $religionPercentages[] = $rel['percentage'];
                                    $religionColors[] = $rel['color'];
                                }
                                $gradientStops = [];
                                $currentPercent = 0;
                                foreach ($religionPercentages as $index => $percent) {
                                    $gradientStops[] = $religionColors[$index] . ' ' . $currentPercent . '% ' . ($currentPercent + $percent) . '%';
                                    $currentPercent += $percent;
                                }
                                $gradientString = implode(', ', $gradientStops);
                            @endphp
                            <div class="relative w-52 h-52">
                                <div class="absolute inset-0 rounded-full" style="background: conic-gradient({{ $gradientString }});"></div>
                                <div class="absolute inset-5 bg-white rounded-full flex items-center justify-center">
                                    <div class="text-center">
                                        <div class="text-3xl font-semibold text-slate-900" data-counter="{{ $totalPop }}">0</div>
                                        <div class="text-sm text-slate-500">Total Penduduk</div>
                        ...
                        <div class="space-y-3">
                            @foreach($religionData as $rel)
                            <div class="flex items-center justify-between p-3 rounded-lg border border-slate-100 bg-slate-50/70">
                                <div class="flex items-center gap-3 text-slate-700">
                                    <span class="legend-dot" style="background-color: {{ $rel['color'] }}"></span>
                                    <span class="font-medium">{{ $rel['label'] }}</span>
                                </div>
                                <div class="text-right">
                                    <span class="text-sm font-semibold" style="color: {{ $rel['color'] }}">{{ $rel['percentage'] }}%</span>
                                    <span class="text-xs text-slate-500 ml-1" data-counter="{{ $rel['value'] }}">(0)</span>
                                </div>
                            </div>
                            @endforeach
                        </div>
                    ...
                    <div class="chart-container" data-aos="fade-left" data-aos-delay="120">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-2xl font-semibold text-slate-900">Distribusi Tingkat Pendidikan</h3>
                            <span class="data-chip">Pendidikan</span>
                        </div>
                        <div class="space-y-4 mb-8">
                            @php
                                $educationData = $chartData['education_distribution'] ?? collect();
                                $educationColors = [
                                    'tidak_sekolah' => '#dc2626',
                                    'sd' => '#ea580c',
                                    'smp' => '#f59e0b',
                                    'sma' => '#10b981',
                                    'diploma' => '#3b82f6',
                            ...
                            <div class="flex items-center gap-4">
                                <div class="w-32 text-sm font-medium text-slate-600">{{ $edu['label'] }}</div>
                                <div class="flex-1 h-3 rounded-full bg-slate-100">
                                    <div class="h-full rounded-full" style="width: {{ $edu['percentage'] }}%; background-color: {{ $educationColors[$edu['key']] ?? '#6b7280' }};"></div>
                                </div>
                                <div class="w-20 text-sm font-semibold text-slate-600 text-right" data-counter="{{ $edu['value'] }}">0</div>
                            </div>
                            @endforeach
                        </div>
                        <div class="p-4 rounded-xl bg-violet-50 border border-violet-100">
                            <p class="text-sm text-violet-700 font-medium">
                                Data pendidikan berdasarkan registrasi penduduk aktif. Tingkat pendidikan yang memadai mendukung pengembangan SDM dan inovasi di desa.
                            </p>
                        </div>
                    </div>
                </div>
