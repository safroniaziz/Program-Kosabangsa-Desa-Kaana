...
<script>
document.addEventListener('DOMContentLoaded', function() {
    // Initialize map centered on Desa Kaana, Enggano
    const map = L.map('map').setView([-5.3502, 102.2615], 13);
    ...
    // Custom marker icons using HTML and Tailwind CSS
    const createCustomIcon = (color, emoji) => {
        return L.divIcon({
            className: 'custom-marker',
            html: `<div class="w-10 h-10 ${color} rounded-full flex items-center justify-center text-white shadow-lg border-2 border-white transform transition-transform hover:scale-110" style="font-size: 16px;">${emoji}</div>`,
            iconSize: [40, 40],
            iconAnchor: [20, 40],
            popupAnchor: [0, -40]
        });
    ...
    // Main marker for Desa Kaana
    const mainMarker = L.marker([-5.3502, 102.2615], {
        icon: createCustomIcon('bg-gradient-to-br from-blue-500 to-indigo-600', '🏘️')
    }).addTo(map);
    ...
    mainMarker.bindPopup(`
        <div class="p-4 min-w-64">
            <div class="text-center border-b border-gray-200 pb-3 mb-3">
                ...
                <p class="text-sm text-blue-600 font-medium">Pulau Enggano, Bengkulu Utara</p>
            </div>
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    <span class="text-gray-600">📍 Koordinat:</span>
                    <span class="font-medium">5°21'S, 102°16'E</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">👥 Penduduk:</span>
                    <span class="font-medium">±1.250 jiwa</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">🏠 Keluarga:</span>
                    <span class="font-medium">329 KK</span>
                </div>
                <div class="flex justify-between">
    ...
    // Important locations with beautiful popups
    const locations = [
        {
            coords: [-5.3505, 102.2620],
            title: "Kantor Desa",
        ...
        {
            coords: [-5.3500, 102.2610],
            title: "Puskesmas Kaana",
            description: "Fasilitas Kesehatan Utama",
            emoji: "🏥",
            color: "bg-gradient-to-br from-green-500 to-emerald-600",
            details: "Pelayanan kesehatan 24 jam dengan 2 dokter"
        },
        {
            coords: [-5.3510, 102.2625],
            title: "SDN Kaana",
            description: "Sekolah Dasar Negeri",
            emoji: "🏫",
            color: "bg-gradient-to-br from-yellow-500 to-amber-600",
            details: "Melayani 156 siswa dengan 8 guru"
        },
        {
            coords: [-5.3495, 102.2605],
            title: "Pelabuhan Kaana",
            description: "Akses Transportasi Laut",
            emoji: "⚓",
            color: "bg-gradient-to-br from-cyan-500 to-blue-600",
            details: "Kapal dari Bengkulu 2x seminggu"
        },
        {
            coords: [-5.3515, 102.2630],
    ...
    locations.forEach(location => {
        const marker = L.marker(location.coords, {
            icon: createCustomIcon(location.color, location.emoji)
        }).addTo(map);

        marker.bindPopup(`
            <div class="p-4 min-w-56">
                <h4 class="font-bold text-lg text-gray-900 flex items-center mb-2">
                    <span class="mr-2 text-xl">${location.emoji}</span>
                    ${location.title}
                </h4>
    ...
    // Toggle buttons functionality with better styling
    ...
    satelliteBtn.addEventListener('click', function() {
        ...
        satelliteBtn.classList.remove('bg-white', 'border-blue-200');
        satelliteBtn.classList.add('bg-blue-100', 'border-blue-300');
        streetBtn.classList.remove('bg-blue-100', 'border-blue-300');
        streetBtn.classList.add('bg-white', 'border-blue-200');
    });
    ...
    streetBtn.addEventListener('click', function() {
        ...
        streetBtn.classList.remove('bg-white', 'border-blue-200');
        streetBtn.classList.add('bg-blue-100', 'border-blue-300');
        satelliteBtn.classList.remove('bg-blue-100', 'border-blue-300');
        satelliteBtn.classList.add('bg-white', 'border-blue-200');
    });
    ...
    console.log('🗺️ Beautiful Leaflet Map initialized for Desa Kaana, Enggano');
});
</script>
@endsection
