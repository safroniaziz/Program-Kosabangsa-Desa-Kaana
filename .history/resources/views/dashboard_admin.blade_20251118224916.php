...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (genderCtx) {
        const genderData = @json($chartData['genderStats']);
        ...
        // Check if we have data
        if (genderValues.length === 0 || genderValues.every(val => val === 0)) {
            genderCtx.parentElement.innerHTML =
                '<div class="text-center py-8"><i class="ki-duotone ki-chart-pie fs-2x text-muted mb-4"></i><p class="text-muted">Belum ada data gender</p></div>';
        } else {
            new Chart(genderCtx.getContext('2d'), {
                ...
                data: {
                    ...
                    datasets: [{
                        data: genderValues,
                        backgroundColor: [
                            'rgba(54, 153, 255, 0.8)',     // Biru - Laki-laki
                            'rgba(255, 99, 132, 0.8)'     // Merah - Perempuan
                        ],
                        borderColor: [
                            'rgba(54, 153, 255, 1)',
                            'rgba(255, 99, 132, 1)'
