...
<!-- Results Modal -->
<div id="resultsModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 items-center justify-center p-4" style="display: none;">
    <div class="bg-white rounded-2xl max-w-md w-full max-h-[85vh] flex flex-col shadow-2xl transform scale-95 opacity-0 transition-all duration-300" id="modalContent">
...
<div id="chatConseling" class="fixed top-0 right-0 h-full w-96 bg-white shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out z-50 border-l border-gray-200 flex flex-col">
    ...
    <!-- Chat Messages Area -->
    <div id="chatMessages" class="flex-1 overflow-y-auto p-3 bg-gray-50" style="height: 300px;">
        <!-- Welcome Message -->
        <div class="mb-3">
    ...
    <div class="p-2 bg-gray-100 border-t" style="max-height: 140px; overflow-y: auto;">
    ...
    <div class="p-3 bg-white border-t border-gray-200">
        <div class="flex items-center space-x-2">
            <input type="text" id="chatInput" placeholder="Ketik pesan..." class="flex-1 p-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-orange-500 focus:border-transparent text-sm">
...
$(document).ready(function() {
    // Assessment Data Structure - Make it globally available
    ...
    function showResults(score) {
        ...
        const resultsHTML = `
            ...
            <div class="flex items-center justify-between p-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Hasil Assessment</h2>
            ...
            <div class="flex-1 overflow-y-auto p-4">
                ...
                <div class="text-center mb-4">
                    <div class="status-indicator ${statusClass} mb-3 mx-auto">
                        <span>${statusIcon}</span>
                    </div>
                    <h3 class="result-title text-${statusClass === 'status-aman' ? 'green' : statusClass === 'status-waspada' ? 'yellow' : 'red'}-600 text-lg font-bold">${status}</h3>
                ...
                <div class="bg-gray-50 rounded-lg p-3 mb-4">
                    <div class="grid grid-cols-3 gap-3 text-center">
                        <div>
                            <div class="text-lg font-bold text-blue-600">${Math.round((score/3) * 100)}%</div>
                            <div class="text-xs text-gray-600">Skor</div>
                        </div>
                        <div>
                            <div class="text-lg font-bold text-green-600">${Object.keys(answers).length}</div>
                            <div class="text-xs text-gray-600">Soal</div>
                        </div>
                        <div>
                            <div class="text-lg font-bold text-purple-600">${assessmentData.sections.length}</div>
                            <div class="text-xs text-gray-600">Bagian</div>
                        </div>
                    </div>
                </div>

                <!-- Description -->
                <div class="text-center text-sm text-gray-700 mb-4 px-2">
                ...
                ${needsCounseling ? `
                    <div class="recommendation-card mb-4">
                        ...
                        <h3 class="text-sm font-bold text-orange-900 mb-1 text-center">💬 Konsultasi Virtual</h3>
                        <p class="text-orange-800 text-center text-xs mb-2">
                            Disarankan berkonsultasi dengan konselor profesional.
                        </p>
                    </div>
                ` : `
                    <div class="recommendation-card-success mb-4">
                        <div class="flex items-center justify-center mb-2">
                        ...
                        <h3 class="text-sm font-bold text-green-900 mb-1 text-center">🎉 Luar Biasa!</h3>
                        <p class="text-green-800 text-center text-xs">
                            Kesiapan mental Anda sudah sangat baik.
                        </p>
                    </div>
                `}
            </div>
            ...
            <div class="border-t border-gray-200 bg-gray-50 rounded-b-2xl">
                <div class="action-buttons space-y-3 p-4">
                    <button onclick="downloadPDFReport()" class="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white shadow-lg hover:shadow-xl px-4 py-3 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-all duration-300 transform hover:scale-105">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                        </svg>
                        <span class="truncate">Download Laporan PDF</span>
                    </button>

                    ${needsCounseling ? `
                        <button onclick="openChatConseling(); closeModal();" class="w-full bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white shadow-lg hover:shadow-xl px-4 py-3 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-all duration-300 transform hover:scale-105">
                            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                            </svg>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button onclick="window.location.href='/'" class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-lg hover:shadow-xl px-3 py-3 rounded-lg text-xs font-medium flex items-center justify-center gap-1 transition-all duration-300 transform hover:scale-105">
                            <svg class="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                            </svg>
                            <span class="hidden sm:inline truncate">Beranda</span>
                            <span class="sm:hidden">Home</span>
                        </button>
                        <button onclick="location.reload()" class="bg-white border-2 border-gray-300 text-gray-700 hover:bg-gray-50 hover:border-gray-400 shadow-md hover:shadow-lg px-3 py-3 rounded-lg text-xs font-medium flex items-center justify-center gap-1 transition-all duration-300 transform hover:scale-105">
                            <svg class="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        $('#resultsModal').removeClass('hidden').css('display', 'flex');
    ...
    window.updateQuickResponses = function() {
        ...
        if (assessmentContext) {
            if (assessmentContext.averageScore < 1.5) {
                ...
                quickResponses = [
                    ...
                    { text: '🔒 Isolasi diri', message: 'Saya cenderung menarik diri dan menghindari diskusi tentang bencana' },
                    { text: '😟 Kecemasan berlebihan', message: 'Saya selalu merasa cemas berlebihan tentang kemungkinan bencana terjadi' }
                ];
                ...
                quickResponses = [
                    ...
                    { text: '🎯 Setting prioritas', message: 'Bagaimana menetapkan prioritas persiapan mental yang realistis untuk pemula?' },
                    { text: '💪 Membangun kepercayaan diri', message: 'Bagaimana membangun kepercayaan diri dalam menghadapi situasi darurat?' }
                ];
                ...
                quickResponses = [
                    ...
                    { text: '🎭 Simulasi mental', message: 'Bagaimana melakukan simulasi mental untuk menghadapi berbagai skenario bencana?' },
                    { text: '👥 Membantu orang lain', message: 'Bagaimana saya bisa membantu orang lain yang kurang siap secara mental?' }
                ];
            } else {
                ...
                quickResponses = [
                    ...
                    { text: '🎖️ Menjadi mentor', message: 'Bagaimana langkah-langkah menjadi mentor konseling tanggap bencana yang efektif?' },
                    { text: '📚 Mengembangkan kurikulum', message: 'Bagaimana menyusun kurikulum pelatihan kesiapan mental tanggap bencana?' }
                ];
            }
        } else {
            ...
            quickResponses = [
                ...
                { text: '👥 Konseling kelompok', message: 'Bagaimana cara membentuk dan menjalankan konseling kelompok untuk kesiapan bencana?' },
                { text: '🧘‍♂️ Konseling mandiri', message: 'Apa saja teknik konseling mandiri yang bisa saya praktikkan sendiri?' }
            ];
        }

        // Tambahkan quick responses umum yang selalu ada - diperbanyak dan lebih spesifik
        const universalResponses = [
            ...
            { text: '🧘‍♀️ Mindfulness bencana', message: 'Ajari saya teknik mindfulness yang bisa diterapkan saat situasi darurat' },
            { text: '💬 Komunikasi efektif', message: 'Bagaimana berkomunikasi secara efektif dengan keluarga saat terjadi bencana?' },
            { text: '🎭 Manajemen emosi', message: 'Bagaimana cara mengelola emosi negatif dan panik saat menghadapi bencana?' },
            { text: '⚡ Respon cepat', message: 'Apa yang harus dilakukan pertama kali ketika mendengar peringatan bencana?' },
            { text: '🔄 Recovery trauma', message: 'Bagaimana cara pulih dari trauma pasca bencana dan membangun kembali mental yang kuat?' },
            { text: '👨‍👩‍👧‍👦 Dinamika keluarga', message: 'Bagaimana menjaga stabilitas mental keluarga selama dan setelah bencana?' },
            { text: '🏢 Konseling workplace', message: 'Bagaimana menerapkan konseling tanggap bencana di lingkungan kerja atau sekolah?' },
            { text: '📈 Monitoring progress', message: 'Bagaimana cara memantau dan mengevaluasi progress kesiapan mental secara berkala?' }
        ];

        // Gabungkan responses dengan universal responses (acak 4 dari total)
        const allResponses = [...quickResponses, ...universalResponses];
        const selectedResponses = shuffleArray(allResponses).slice(0, 5);
    ...
    window.closeModal = function() {
        $('#resultsModal').addClass('hidden').css('display', 'none');
    };

    // Download PDF Report function
    window.downloadPDFReport = function() {
        const assessmentData = JSON.parse(localStorage.getItem('assessmentAnswers')) || {};
        const assessmentContext = getAssessmentContext();

        // Prepare data for PDF generation
        const reportData = {
            timestamp: new Date().toISOString(),
            participantName: 'Peserta Assessment', // You can add name input if needed
            totalQuestions: totalQuestions,
            answers: assessmentData,
            context: assessmentContext,
            sections: window.assessmentData ? window.assessmentData.sections : []
        };

        // Store data for PDF generation
        sessionStorage.setItem('pdfReportData', JSON.stringify(reportData));

        // Open PDF generation page in new tab
        const pdfUrl = `/assessment/pdf-report?data=${encodeURIComponent(JSON.stringify(reportData))}`;
