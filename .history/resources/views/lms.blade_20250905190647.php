...
<style>
/* Material List Styles - replacing dynamic Tailwind */
.material-item {
    @apply flex items-start gap-3 p-3 rounded-lg cursor-pointer transition-all duration-200 hover:bg-slate-100;
}
...
.active-indicator {
    @apply w-3 h-3 bg-blue-500 rounded-full flex-shrink-0 mt-1;
}

/* Toast notification styles */
.toast {
    @apply fixed bottom-6 right-6 text-white px-6 py-4 rounded-xl shadow-lg z-50 transform translate-y-full opacity-0 transition-all duration-300;
}
...
.video-player-container {
    @apply w-full h-full bg-gradient-to-br from-slate-900 to-black flex items-center justify-center;
}

.video-wrapper {
    @apply w-full max-w-5xl mx-auto p-8;
}
...
.video-message {
    @apply text-white/80 text-lg mb-4;
}

.back-to-material-btn {
    @apply px-6 py-3 bg-white/10 backdrop-blur-sm text-white rounded-xl border border-white/20 hover:bg-white/20 transition-all;
}
</style>
@endsection
                ...
                <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 group module-card"
                    ...
                    <div class="p-6 bg-white">
                        ...
                        <div class="flex items-center justify-between">
                            @if($module['status'] === 'available')
                            <div class="flex-1">
                                <button onclick="openModule({{ $module['number'] }}, @json($module))" class="w-full btn-primary text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg transition-all group relative overflow-hidden">
...
// Populate materials list in sidebar - simplified without heavy Tailwind classes
function populateMaterialsList() {
    ...
    currentMaterials.forEach((material, index) => {
        ...
        // Use CSS classes instead of inline Tailwind
        materialItem.className = `material-item ${index === currentMaterialIndex ? 'active' : ''}`;
        materialItem.onclick = () => loadMaterial(index);

        // Simplified HTML structure
        materialItem.innerHTML = `
            <div class="material-icon ${material.type === 'text' ? 'text-icon' : 'video-icon'}">
                ${material.type === 'text'
            ...
            <div class="material-info">
                ...
                <div class="material-duration">${material.duration}</div>
                <div class="material-badges">
                    ${material.optional_video ? '<span class="badge video-badge">Video</span>' : ''}
                    ${material.optional_file ? '<span class="badge file-badge">File</span>' : ''}
                </div>
            </div>
            ${index === currentMaterialIndex ? '<div class="active-indicator"></div>' : ''}
        `;
...
function getEnhancedContent(title) {
    const contents = {
        'Pengenalan HTML5': `
            ...
            <p class="mb-6">
                HTML5 adalah versi terbaru dari HTML (HyperText Markup Language) yang merupakan standar markup untuk membuat halaman web.
                HTML5 memberikan struktur semantik yang lebih baik, dukungan multimedia native, dan API yang lebih kaya untuk pengembangan aplikasi web modern.
            </p>

            <div class="bg-slate-50 border border-slate-200 rounded-xl p-6 mb-6">
                <h5 class="font-bold text-slate-900 mb-3">Keunggulan HTML5:</h5>
                <ul class="space-y-2 text-slate-700">
                    <li>• Semantic elements yang lebih bermakna</li>
                    <li>• Dukungan audio dan video native</li>
                    <li>• Canvas untuk graphics dan animasi</li>
                    <li>• Local storage dan offline capabilities</li>
                    <li>• Responsive design yang lebih mudah</li>
                </ul>
            </div>
        ...
        'Semantic Elements': `
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div class="bg-blue-50 border border-blue-200 p-4 rounded-lg">
                    ...
                    <p class="text-blue-800 text-sm">Untuk bagian kepala/header halaman</p>
                </div>
                <div class="bg-green-50 border border-green-200 p-4 rounded-lg">
                    <h5 class="font-bold text-green-900 mb-2">&lt;nav&gt;</h5>
                    <p class="text-green-800 text-sm">Untuk navigasi utama</p>
                </div>
                <div class="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                    <h5 class="font-bold text-yellow-900 mb-2">&lt;main&gt;</h5>
                    <p class="text-yellow-800 text-sm">Untuk konten utama halaman</p>
                </div>
                <div class="bg-purple-50 border border-purple-200 p-4 rounded-lg">
                    <h5 class="font-bold text-purple-900 mb-2">&lt;footer&gt;</h5>
                    <p class="text-purple-800 text-sm">Untuk bagian bawah halaman</p>
                </div>
            </div>
...
// Show video in modal with enhanced player - cleaned up, using template
function showVideo(videoUrl) {
    const contentArea = document.getElementById('contentArea');
    const template = document.getElementById('videoPlayerTemplate');
    const content = template.content.cloneNode(true);
    
    // Set video URL with parameters
    content.querySelector('.video-iframe').src = `${videoUrl}?autoplay=1&rel=0&modestbranding=1&showinfo=0`;
    
    // Set back button action
    content.querySelector('.back-to-material-btn').onclick = () => loadMaterial(currentMaterialIndex);
...
// Download file function - cleaned up, using template
function downloadFile(filename) {
    ...
    iconPath.setAttribute('d', 'M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z');
    toast.querySelector('.toast-message').textContent = `Mengunduh ${filename}...`;
    ...
    setTimeout(() => {
        ...
        toast.querySelector('.toast-message').textContent = `Download ${filename} berhasil!`;
