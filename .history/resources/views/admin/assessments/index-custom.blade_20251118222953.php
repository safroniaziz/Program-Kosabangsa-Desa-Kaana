...
<script>
    ...
    function loadAssessments() {
        ...
        $.get(`{{ route('admin.assessments.data') }}?${params}`)
            ...
            .fail(function(xhr) {
                let errorMsg = 'Gagal memuat data assessment';
    ...
    $(document).on('click', '.btn-view', function () {
        ...
        $.get(`{{ route('admin.assessments.show', ':id') }}`.replace(':id', id))
            ...
            .fail(function () {
                showAlert('error', 'Gagal memuat detail assessment');
                hideLoader();
            });
    });
    ...
    $(document).on('click', '.btn-history', function () {
        ...
        // Build URL manually to avoid route parameter issues
        let url = '{{ url("/admin/assessments/history") }}/' + userId + '/' + type;
        ...
        $.ajax({
            ...
            error: function (xhr) {
                let errorMsg = 'Gagal memuat riwayat assessment';
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    errorMsg = xhr.responseJSON.message;
                }
                showAlert('error', errorMsg);
