            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div class="chart-container" data-aos="fade-right" data-aos-delay="120">
                        ...
                        <div class="flex items-end justify-center gap-4 p-6 bg-slate-50 rounded-xl mb-6 overflow-x-auto">
                            @forelse($orderedEducation as $education)
                                @php
                                    $barHeight = $maxEducationValue > 0 ? ($education['value'] / $maxEducationValue) * 100 : 0;
                                    $barColor = $educationColors[$education['key']] ?? '#3b82f6';
                                @endphp
                                <div class="flex flex-col items-center gap-2 px-2 flex-1 max-w-[100px]">
                                    <div class="relative w-full bg-slate-200 rounded-t" style="height: 200px;">
                                        <div class="absolute bottom-0 left-0 w-full rounded-t" style="height: {{ $barHeight }}%; background-color: {{ $barColor }};"></div>
                                        <span class="absolute -top-7 left-1/2 -translate-x-1/2 text-sm font-semibold text-slate-700 whitespace-nowrap">{{ number_format($education['value']) }}</span>
                                    </div>
                                    <span class="text-sm font-medium text-slate-600 text-center">{{ $education['label'] }}</span>
                                    <span class="text-xs font-semibold" style="color: {{ $barColor }};">{{ rtrim(rtrim(number_format($education['percentage'], 1), '0'), '.') }}%</span>
                                </div>
                            @empty
