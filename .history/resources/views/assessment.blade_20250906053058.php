...
$(document).ready(function() {
    ...
    function buildNavigation() {
        ...
        assessmentData.sections.forEach((section, sectionIndex) => {
            section.questions.forEach((question, qIndex) => {
                const questionItem = $(`
                    <div class="question-nav-item p-2 rounded text-xs cursor-pointer transition-all duration-200 ${questionIndex === 1 ? 'active' : ''}" data-section="${sectionIndex}" data-question="${qIndex}">
                        <span class="font-medium">Q${questionIndex}</span>
                    </div>
                `);
    ...
    function updateNavigationStates() {
        ...
        $('.question-nav-item').each(function() {
            ...
            $(this).removeClass('active completed bg-blue-100 bg-green-100 text-blue-600 text-green-600');

            if (isAnswered) {
                $(this).addClass('completed bg-green-100 text-green-600');
            } else if (isActive) {
                $(this).addClass('active bg-blue-100 text-blue-600');
            }
        });
    }
    ...
    // Navigation clicks
    $(document).on('click', '.section-nav-item', function() {
        const sectionIndex = $(this).data('section');
        jumpToQuestion(sectionIndex, 0);
    });
    ...
    // Show Results (same as before but enhanced)
    function showResults(score) {
        ...
        // Enhanced results modal with detailed analysis
        const resultsHTML = `
            <div class="text-center relative">
                <button onclick="$('#resultsModal').addClass('hidden')" class="absolute top-0 right-0 text-gray-400 hover:text-gray-600 transition-colors">
                ...
                <div class="mb-8">
                    <h3 class="text-3xl font-bold text-gray-900 mb-3">Hasil Assessment Kesiapan Mental</h3>
                    <div class="bg-gray-100 rounded-xl p-4 inline-block">
                        <p class="text-gray-700"><strong>Nama:</strong> Ahmad Wijaya</p>
                        <p class="text-gray-700"><strong>Usia:</strong> 32 tahun</p>
                        <p class="text-gray-700"><strong>Tanggal:</strong> ${new Date().toLocaleDateString('id-ID')}</p>
                        <p class="text-gray-700"><strong>Total Pertanyaan:</strong> ${totalQuestions}</p>
                    </div>
                </div>
                ...
                <div class="relative mx-auto mb-8" style="width: 200px; height: 200px;">
                    <svg class="w-full h-full progress-ring" viewBox="0 0 100 100">
                        ...
                        <circle cx="50" cy="50" r="45" stroke="url(#gradient-${status.toLowerCase()})" stroke-width="8" fill="none" 
                                stroke-dasharray="283" stroke-dashoffset="${283 - (score/3) * 283}" 
                    ...
                    <div class="absolute inset-0 flex items-center justify-center">
                        <div class="${statusClass} w-20 h-20 rounded-full flex items-center justify-center text-white text-3xl font-bold shadow-lg">
                            ${statusIcon}
                        </div>
                    </div>
                </div>
                
                <h3 class="text-4xl font-bold text-gray-900 mb-3">Status: ${status}</h3>
                <p class="text-lg text-gray-600 mb-6">${additionalInfo}</p>
                
                <!-- Score Details -->
                <div class="bg-gray-50 rounded-2xl p-6 mb-6">
                    <h4 class="font-bold text-gray-900 mb-4">Detail Skor Assessment</h4>
                    <div class="grid grid-cols-3 gap-4">
                        <div class="text-center">
                            <div class="text-3xl font-bold text-blue-600">${score.toFixed(1)}</div>
                            <div class="text-sm text-gray-600">Skor Rata-rata</div>
                        </div>
                        <div class="text-center">
                            <div class="text-3xl font-bold text-green-600">${Math.round((score/3)*100)}%</div>
                ...
                <div class="text-left bg-gray-50 rounded-2xl p-6 mb-6">
                    <h4 class="font-bold text-gray-900 mb-3">🎯 Rekomendasi Personal</h4>
                ...
                ${needsCounseling ? `
                    <div class="bg-gradient-to-r from-orange-50 to-red-50 border-2 border-orange-200 rounded-2xl p-6 mb-6">
                        <div class="flex items-center justify-center mb-4">
                            ...
                            <div class="text-left">
                                <h4 class="font-bold text-orange-900 text-lg">⚠️ Konseling Direkomendasikan</h4>
                                <p class="text-orange-800">Tim konselor profesional kami siap membantu Anda</p>
                            </div>
                        </div>
                        <button onclick="window.location.href='/konseling'" class="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white px-6 py-4 rounded-xl font-bold text-lg hover:from-orange-600 hover:to-red-600 transition-all duration-300 transform hover:scale-105">
                            💬 Mulai Chat Konseling Sekarang
                        </button>
                    </div>
                ` : `
                    <div class="bg-green-50 border-2 border-green-200 rounded-2xl p-6 mb-6">
                        <h4 class="font-bold text-green-900 mb-3">✅ Status Mental Baik</h4>
