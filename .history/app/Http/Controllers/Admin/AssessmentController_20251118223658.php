        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            return response()->json([
                'data' => $results->map(function ($assessment) {
                    ...
                    // Action button - only history (link to new page)
                    $typeForRoute = $assessment->assessment_type === 'checklist_masalah' ? 'dcm' : $assessment->assessment_type;
                    $historyUrl = route('admin.assessments.history', ['userId' => $assessment->user_id, 'type' => $typeForRoute]);
                    $action = '<a href="' . $historyUrl . '" class="btn btn-sm btn-primary" title="Lihat Riwayat">
    ...
    public function getHistory(Request $request, $userId, $type)
    {
        try {
            ...
            $assessments = UserAssessment::with('user')
                ...
                ->where('assessment_type', $dbType)
                ->where('status', 'completed')
                ->orderBy('completed_at', 'desc')
                ->orderBy('created_at', 'desc')
                ->get();

            return view('admin.assessments.history', compact('assessments', 'type', 'user'));
        } catch (\Exception $e) {
            Log::error('AssessmentController getHistory error: ' . $e->getMessage());
            return redirect()->route('admin.assessments.index')
                ->with('error', 'Gagal memuat riwayat assessment: ' . $e->getMessage());
        }
    }
