...
class AssessmentController extends Controller
{
    ...
    public function getAssessments(Request $request)
    {
        ...
        if ($request->filled('risk_level') && $request->input('risk_level') !== '') {
            $baseQuery->where('risk_level', $request->input('risk_level'));
        }
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            $latestAssessments = $baseQuery
                ->selectRaw('user_assessments.*, ROW_NUMBER() OVER (PARTITION BY user_id, assessment_type ORDER BY created_at DESC) as rn')
                ->get()
            ...
            $sorted = $latestAssessments->sortBy(function($item) use ($sortBy, $sortOrder) {
                return $item->$sortBy;
            });
            ...
            $total = $sorted->count();
            $results = $sorted->skip(($page - 1) * $perPage)->take($perPage);
            ...
            return response()->json([
                'data' => $results->map(function ($assessment) {
                    ...
                    $action = '<div class="btn-group" role="group">
                        <button type="button" class="btn btn-sm btn-info btn-view" data-id="' . $assessment->id . '" title="View">
