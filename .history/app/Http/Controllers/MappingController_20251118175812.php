...
class MappingController extends Controller
{
    ...
    private function calculateDevelopmentIndices($totalUsers, $socioeconomicDistribution, $educationDistribution, $ageGroupDistribution)
    {
        ...
        // 2. Pendidikan Formal (Persentase) - warga yang memiliki pendidikan formal
        // Sumber: field education_level di tabel users
        // Menghitung persentase warga yang pernah mengenyam pendidikan formal (SD, SMP, SMA, Diploma, Sarjana, Pascasarjana)
        // Hanya menghitung dari user yang sudah mengisi education_level
        $usersWithEducation = User::where('role', 'user')
            ->whereNotNull('education_level')
            ->count();

        $formalEducationCount = 0;
        foreach ($educationDistribution as $education) {
            if (($education['key'] ?? '') !== 'tidak_sekolah') {
                $formalEducationCount += $education['value'] ?? 0;
            }
        }
        // Persentase dihitung dari total user yang sudah mengisi education_level, bukan semua user
        $formalEducationPercentage = $usersWithEducation > 0 ? round(($formalEducationCount / $usersWithEducation) * 100, 1) : 0;
        ...
        // 4. Kualitas SDM (Skor /100) - rata-rata dari 3 indikator utama
        // Formula: (Kesejahteraan + Pendidikan Formal + Usia Produktif) / 3
        // Semua data dihitung dari tabel users yang terdaftar di sistem
        $humanResourceQuality = round(($welfarePercentage + $formalEducationPercentage + $workParticipationPercentage) / 3, 0);
        ...
        // Determine status label berdasarkan kualitas SDM
        $statusLabel = 'Sedang';
        $statusColor = 'yellow';
        if ($humanResourceQuality >= 75) {
            $statusLabel = 'Baik';
            $statusColor = 'green';
        } elseif ($humanResourceQuality < 50) {
            $statusLabel = 'Perlu Perhatian';
            $statusColor = 'red';
        }

        return [
            'human_resource_quality' => $humanResourceQuality,
            'welfare_percentage' => $welfarePercentage,
            'formal_education_percentage' => $formalEducationPercentage,
            'formal_education_count' => $formalEducationCount,
            'users_with_education_data' => $usersWithEducation,
