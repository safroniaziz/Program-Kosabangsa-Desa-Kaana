...
class AssessmentController extends Controller
{
    ...
    public function submitDcmChecklist(Request $request, Assessment $assessment)
    {
        try {
            ...
            // Get responses/answers from request (DCM uses 'responses', PTSD uses 'answers')
            $responses = $request->responses ?? $request->answers ?? [];
            ...
            for ($problemId = 1; $problemId <= $totalProblems; $problemId++) {
                // If checkbox is checked in answers array, value is 1, otherwise 0
                $answerValue = isset($answers[$problemId]) && $answers[$problemId] == 1 ? 1 : 0;
