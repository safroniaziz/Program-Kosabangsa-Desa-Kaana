...
class AssessmentController extends Controller
{
    ...
    public function submitDcm(Request $request, Assessment $assessment)
    {
        ...
        try {
            ...
            return redirect()->route('assessment.start-new', $assessment)
                ->with('success', 'Assessment DCM berhasil diselesaikan. Anda dapat melihat hasil pada panel riwayat.');
    ...
    {
        // Validation untuk checkbox format - tidak wajib semua harus dijawab
        // If answers is empty array, skip validation
        if (empty($request->answers) || !is_array($request->answers)) {
            $request->merge(['answers' => []]);
        }
        
        $request->validate([
            'answers' => 'array', // Array is optional
            'answers.*' => 'sometimes|integer|min:0|max:1'
        ]);

        try {
            DB::beginTransaction();

            Log::info('DCM Checklist submission data:', [
                'assessment_type' => $assessment->type,
                'answers' => $request->answers ?? [],
                'user_id' => Auth::id()
            ]);
            ...
            // Prepare responses array (problem_id => value)
            $responses = [];
            ...
            for ($problemId = 1; $problemId <= $totalProblems; $problemId++) {
                ...
                $answerValue = isset($request->answers[$problemId]) && $request->answers[$problemId] == 1 ? 1 : 0;
                $responses[$problemId] = $answerValue;
            ...
            $userAssessment->update([
                ...
                'overall_risk_level' => $calculationResults['risk_level'] ?? 'low',
                'interpretation_text' => 'DCM Assessment - ' . ucfirst($calculationResults['risk_level'] ?? 'low') . ' Risk',
                'recommendations' => $calculationResults['recommendations'] ?? [],
                'requires_followup' => in_array($calculationResults['risk_level'] ?? 'low', ['moderate', 'high'])
            ]);
            ...
            Log::info('DCM Transaction committed, redirecting to result page...');

            return redirect()->route('assessment.result', ['userAssessment' => $userAssessment->id])
                ->with('success', 'Assessment DCM Checklist berhasil diselesaikan!');
