...
class AssessmentController extends Controller
{
    ...
    private function calculateAndUpdateDcmResults(UserAssessment $userAssessment)
    {
        ...
        // Map Indonesian risk level to English enum values
        $riskLevelMap = [
            'rendah' => 'low',
            'sedang' => 'moderate',
            'tinggi' => 'high',
        ...
        $riskLevel = strtolower($calculationResults['risk_level'] ?? 'rendah');
        $overallRiskLevel = $riskLevelMap[$riskLevel] ?? 'low';

        // Update user assessment with DCM results
        $userAssessment->update([
            'results' => $calculationResults,
            'overall_risk_level' => $overallRiskLevel,
            'interpretation_text' => 'DCM Assessment - ' . ucfirst($calculationResults['risk_level'] ?? 'Rendah') . ' Risk',
            'recommendations' => $calculationResults['recommendations'] ?? [],
            'requires_followup' => in_array($overallRiskLevel, ['moderate', 'high', 'critical'])
        ]);
