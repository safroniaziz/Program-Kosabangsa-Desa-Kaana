    ...
    private function updateDemographicsWithDynamicData($demographics, $chartData)
    {
        return $demographics->map(function ($demo) use ($chartData) {
            ...
            if ($demo->key === 'productive_age') {
                $productiveAgeGroups = ['remaja', 'dewasa_muda', 'dewasa'];
                $productiveCount = 0;
                $totalPop = $chartData['total_population'] ?? 0;
                
                foreach ($chartData['age_group_distribution'] as $ageGroup) {
                    $label = $ageGroup['label'] ?? '';
                    if (in_array($ageGroup['label'], ['Remaja (15-24)', 'Dewasa Muda (25-44)', 'Dewasa (45-64)']) ||
                        in_array($label, $productiveAgeGroups)) {
                        $productiveCount += $ageGroup['value'] ?? 0;
                    }
                }
                
                // Also check by label format
                foreach ($chartData['age_group_distribution'] as $ageGroup) {
                    $label = strtolower($ageGroup['label'] ?? '');
                    if (strpos($label, 'remaja') !== false || 
                        strpos($label, 'dewasa muda') !== false || 
                        strpos($label, 'dewasa') !== false) {
                        $productiveCount += $ageGroup['value'] ?? 0;
                    }
                }
                
                // Calculate from age groups directly
