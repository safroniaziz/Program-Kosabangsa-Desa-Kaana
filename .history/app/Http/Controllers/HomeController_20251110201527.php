...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $totalSmartServices = Coordinate::where(function($query) {
                $query->where('name', 'like', '%Digital%')
                      ->orWhere('name', 'like', '%Sistem%')
                      ->orWhere('name', 'like', '%Layanan%')
                      ->orWhere('name', 'like', '%Pos Pelayanan%')
                      ->orWhere('name', 'like', '%Perpustakaan%')
                      ->orWhere('name', 'like', '%Internet%')
                      ->orWhere('name', 'like', '%Informasi%')
                      ->orWhere('name', 'like', '%Pengaduan%')
                      ->orWhere('name', 'like', '%Pencatatan%')
                      ->orWhere('name', 'like', '%Bantuan%')
                      ...
                      ->orWhere('name', 'like', '%Pasar%')
                      ->orWhere('name', 'like', '%Konsultasi%');
            })
            ->where('description', 'not like', '%UMKM Digital%')
            ->where('name', 'not like', '%Puskesmas%')
            ->where('name', 'not like', '%Klinik%')
            ->where('name', 'not like', '%Pos Kesehatan%')
            ->where('name', 'not like', '%Pusat Pelayanan Kesehatan%')
