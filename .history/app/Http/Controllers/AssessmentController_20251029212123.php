...
class AssessmentController extends Controller
{
    ...
    private function saveDcmAnswers(UserAssessment $userAssessment, array $responses, Assessment $assessment)
    {
        ...
        $totalProblems = $assessment->total_questions ?? 100; // Default 100 for DCM
        ...
        for ($problemId = 1; $problemId <= $totalProblems; $problemId++) {
            ...
            // Check if this problem ID exists in responses and is checked
            if (isset($responses[$problemId]) && $responses[$problemId] == 1) {
                $answerValue = 1;

                // Debug log for checked items
                $problem = \App\Services\AssessmentQuestions\ChecklistMasalahQuestions::getProblem($problemId);
                if ($problem) {
                    Log::info("DCM: Problem {$problemId} ({$problem['category']}: {$problem['problem']}) is CHECKED");
                }
            }
