...
class AssessmentCalculationService
{
    ...
    public function calculateChecklistMasalah(UserAssessment $userAssessment): array
    {
        ...
        $categoryScores = ChecklistMasalahQuestions::calculateCategoryScores($responses);

        // Get dominant category
        $dominantCategory = ChecklistMasalahQuestions::getDominantCategory($responses);

        // Convert numeric category to name for display
        $dominantCategoryName = ChecklistMasalahQuestions::getCategoryName($dominantCategory);

        // Calculate total problems
        $totalProblems = ChecklistMasalahQuestions::calculateTotalProblems($responses);
        $totalPossibleProblems = ChecklistMasalahQuestions::getTotalProblems();

        // Get interpretation
        $interpretation = ChecklistMasalahQuestions::getInterpretation($dominantCategory, $totalProblems);

        // Get recommendations
        $recommendations = ChecklistMasalahQuestions::getRecommendations(
            $dominantCategory,
            $totalProblems
        );
