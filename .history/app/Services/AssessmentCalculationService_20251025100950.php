...
class AssessmentCalculationService
{
    ...
    public function calculatePTSDDiagnostic(UserAssessment $userAssessment): array
    {
        ...
        // Determine risk level based on total score and category analysis
        $riskLevel = $this->getPTSDDiagnosticRisk($summary);
        ...
        $results = [
            ...
            'risk_level' => $riskLevel,
            'recommendations' => $this->getPTSDDiagnosticRecommendations($riskLevel, $summary)
        ];

        // Create alert if high risk
        if ($riskLevel === 'high') {
            $this->createMentalHealthAlert($userAssessment, 'ptsd_diagnostic', $riskLevel, $results);
        }
    ...
    /**
     * Determine risk level for PTSD Diagnostic based on scores and patterns
     */
    private function getPTSDDiagnosticRisk(array $summary): string
    {
        $totalScore = $summary['total_score'];
        $categoryScores = $summary['category_scores'];

        // High risk if total score >= 15 or multiple high category scores
        if ($totalScore >= 15) {
            return 'high';
        }
        ...
        foreach ($categoryScores as $score) {
            if ($score >= 3) { // Lowered threshold for high categories
                $highCategories++;
            }
        }

        if ($totalScore >= 10 || $highCategories >= 2) {
            return 'moderate';
        }

        if ($totalScore >= 5 || $highCategories >= 1) {
            return 'mild';
        }
    ...
    /**
     * Get recommendations for PTSD Diagnostic results
     */
    private function getPTSDDiagnosticRecommendations(string $riskLevel, array $summary): array
    {
        $recommendations = [];
        ...
        switch ($riskLevel) {
            case 'high':
                $recommendations[] = "Sangat disarankan untuk segera berkonsultasi dengan psikolog atau psikiater";
                $recommendations[] = "Pertimbangkan untuk mencari terapi trauma seperti EMDR atau trauma-focused therapy";
                $recommendations[] = "Hindari alkohol dan zat adiktif sebagai pelarian";
                $recommendations[] = "Cari dukungan dari keluarga dan teman terdekat";
                break;
            ...
            case 'moderate':
                ...
                $recommendations[] = "Praktikkan teknik relaksasi dan mindfulness";
                $recommendations[] = "Jaga rutinitas tidur dan olahraga teratur";
                $recommendations[] = "Pertimbangkan bergabung dengan support group";
                break;
        ...
        // Add specific recommendations based on primary concern
        if (isset($summary['primary_concern'])) {
            $primaryConcern = $summary['primary_concern'];
            $categoryName = PTSDDiagnosticQuestions::getCategoryNames()[$primaryConcern] ?? '';

            if ($categoryName) {
                $recommendations[] = "Area utama perhatian: {$categoryName} - " .
                    PTSDDiagnosticQuestions::getCategoryDescription($primaryConcern);
            }
        }
