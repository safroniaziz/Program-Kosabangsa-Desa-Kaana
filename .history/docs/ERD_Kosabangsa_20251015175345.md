...
├── option_value (VARCHAR(100), NULLABLE) -- untuk scoring
├── score_weight (DECIMAL(3,2), DEFAULT 0.00)
├── order_index (INT, NOT NULL)
├── created_at (TIMESTAMP)
└── updated_at (TIMESTAMP)
```
...
├── results_data (JSON, NULLABLE) -- career recommendations, personality profile
├── created_at (TIMESTAMP)
└── updated_at (TIMESTAMP)
```
