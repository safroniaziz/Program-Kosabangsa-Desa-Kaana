...
├── title (VARCHAR(255), NOT NULL)
├── description (TEXT, NULLABLE)
├── category (ENUM('career_guidance', 'skill_assessment', 'personality', 'aptitude', 'mental_health', 'ptsd_screening', 'dass21'), NOT NULL)
├── assessment_type (ENUM('pcl5', 'dass21', 'career', 'personality', 'custom'), NOT NULL)
├── total_questions (INT, NOT NULL)
├── time_limit_minutes (INT, NULLABLE)
├── scoring_method (ENUM('sum_total', 'weighted', 'subscale', 'categorical'), DEFAULT 'sum_total')
├── max_score (INT, NULLABLE) -- 80 for PCL-5, 126 for DASS-21
├── threshold_score (DECIMAL(5,2), NULLABLE) -- 33 for PCL-5 probable PTSD
├── instructions (TEXT, NULLABLE)
├── disclaimer_text (TEXT, NULLABLE) -- Legal disclaimer untuk mental health assessment
├── requires_consent (BOOLEAN, DEFAULT false)
├── is_active (BOOLEAN, DEFAULT true)
├── created_by (FK -> users.id)
├── version (VARCHAR(20), DEFAULT '1.0') -- untuk tracking versi assessment
├── created_at (TIMESTAMP)
└── updated_at (TIMESTAMP)
```
...
├── question_code (VARCHAR(20), NULLABLE) -- PCL1, PCL2, DASS1, etc.
├── question_type (ENUM('likert_scale', 'multiple_choice', 'single_choice', 'text'), NOT NULL)
├── scale_type (ENUM('0_to_4', '0_to_3', '1_to_5', 'custom'), NULLABLE) -- untuk skala Likert
├── cluster_category (VARCHAR(100), NULLABLE) -- Re-experiencing, Avoidance, Depression, Anxiety, Stress
├── subscale (VARCHAR(50), NULLABLE) -- untuk DASS-21: depression/anxiety/stress
├── reverse_scored (BOOLEAN, DEFAULT false) -- untuk pertanyaan yang perlu di-reverse
├── weight (DECIMAL(3,2), DEFAULT 1.00)
├── order_index (INT, NOT NULL)
├── is_required (BOOLEAN, DEFAULT true)
├── help_text (TEXT, NULLABLE) -- penjelasan tambahan untuk pertanyaan
├── created_at (TIMESTAMP)
└── updated_at (TIMESTAMP)
```
...
assessment_options
├── id (PK, BIGINT, AUTO_INCREMENT)
├── question_id (FK -> assessment_questions.id)
├── option_text (TEXT, NOT NULL)
├── option_value (INT, NOT NULL) -- 0,1,2,3,4 untuk PCL-5 atau 0,1,2,3 untuk DASS-21
├── option_description (VARCHAR(255), NULLABLE) -- "Not at all", "A little bit", etc.
├── score_weight (DECIMAL(3,2), DEFAULT 1.00)
├── order_index (INT, NOT NULL)
├── created_at (TIMESTAMP)
└── updated_at (TIMESTAMP)
```
...
├── raw_score (INT, NULLABLE) -- skor mentah sebelum transformasi
├── subscale_scores (JSON, NULLABLE) -- untuk DASS-21: {depression: 14, anxiety: 10, stress: 18}
├── cluster_scores (JSON, NULLABLE) -- untuk PCL-5: {reexperiencing: 12, avoidance: 8, ...}
├── severity_level (ENUM('normal', 'mild', 'moderate', 'severe', 'extremely_severe'), NULLABLE)
├── interpretation (TEXT, NULLABLE) -- interpretasi hasil berdasarkan skor
├── recommendations (JSON, NULLABLE) -- rekomendasi tindak lanjut
├── risk_level (ENUM('low', 'moderate', 'high', 'critical'), NULLABLE)
├── requires_followup (BOOLEAN, DEFAULT false) -- perlu tindak lanjut profesional
├── consent_given (BOOLEAN, DEFAULT false) -- persetujuan untuk assessment mental health
├── status (ENUM('in_progress', 'completed', 'abandoned', 'flagged'), DEFAULT 'in_progress')
├── results_data (JSON, NULLABLE) -- data tambahan hasil assessment
├── counselor_notes (TEXT, NULLABLE) -- catatan konselor jika ada review
├── reviewed_by (FK -> users.id, NULLABLE) -- konselor yang me-review
...
├── question_id (FK -> assessment_questions.id)
├── selected_option_id (FK -> assessment_options.id, NULLABLE) -- untuk single choice
├── selected_options (JSON, NULLABLE) -- array of option IDs untuk multiple choice
├── text_answer (TEXT, NULLABLE)
├── score_value (INT, DEFAULT 0) -- nilai skor mentah (0-4 untuk PCL-5, 0-3 untuk DASS-21)
├── score_earned (DECIMAL(5,2), DEFAULT 0.00) -- skor setelah weighted/transformasi
├── response_time_seconds (INT, NULLABLE) -- waktu menjawab pertanyaan
├── answered_at (TIMESTAMP, NOT NULL)
├── created_at (TIMESTAMP)
└── updated_at (TIMESTAMP)
```

### 14. **ASSESSMENT_SCORING_RULES** (Aturan Scoring)
```sql
assessment_scoring_rules
├── id (PK, BIGINT, AUTO_INCREMENT)
├── assessment_id (FK -> assessments.id)
├── rule_name (VARCHAR(100), NOT NULL) -- "PCL5_PTSD_Threshold", "DASS_Depression_Severe"
├── rule_type (ENUM('threshold', 'range', 'subscale', 'combination'), NOT NULL)
├── condition_field (VARCHAR(50), NOT NULL) -- "total_score", "subscale_depression", etc.
├── condition_operator (ENUM('>=', '>', '<=', '<', '=', 'between'), NOT NULL)
├── condition_value (DECIMAL(8,2), NOT NULL) -- nilai threshold/batas
├── condition_value_max (DECIMAL(8,2), NULLABLE) -- untuk range
├── result_category (VARCHAR(100), NOT NULL) -- "Probable PTSD", "Severe Depression"
├── result_severity (ENUM('normal', 'mild', 'moderate', 'severe', 'extremely_severe'), NULLABLE)
├── result_interpretation (TEXT, NULLABLE)
├── result_recommendations (JSON, NULLABLE)
├── priority_order (INT, DEFAULT 0) -- urutan evaluasi rule
├── is_active (BOOLEAN, DEFAULT true)
├── created_at (TIMESTAMP)
└── updated_at (TIMESTAMP)
```

### 15. **MENTAL_HEALTH_ALERTS** (Alert Kesehatan Mental)
```sql
mental_health_alerts
├── id (PK, BIGINT, AUTO_INCREMENT)
├── user_assessment_id (FK -> user_assessments.id)
├── user_id (FK -> users.id)
├── alert_type (ENUM('high_risk', 'severe_symptoms', 'suicidal_ideation', 'urgent_referral'), NOT NULL)
├── severity_level (ENUM('moderate', 'high', 'critical'), NOT NULL)
├── trigger_score (DECIMAL(5,2), NOT NULL) -- skor yang memicu alert
├── trigger_questions (JSON, NULLABLE) -- pertanyaan spesifik yang memicu alert
├── alert_message (TEXT, NOT NULL)
├── recommended_actions (JSON, NOT NULL) -- tindakan yang direkomendasikan
├── requires_immediate_action (BOOLEAN, DEFAULT false)
├── professional_contact_info (JSON, NULLABLE) -- kontak profesional terdekat
├── status (ENUM('active', 'acknowledged', 'resolved', 'escalated'), DEFAULT 'active')
├── acknowledged_by (FK -> users.id, NULLABLE) -- konselor yang acknowledge
